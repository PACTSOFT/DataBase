...
DECLARE @GENERALTIMINGS TABLE(ID BIGINT IDENTITY(1,1),STARTTIME TIME,ENDTIME TIME,LOCATION INT)
CREATE TABLE #TAB(ID INT IDENTITY(1,1),LFT BIGINT,EMPNODE INT,EMPCODE VARCHAR(1000),EMPNAME VARCHAR(1000),DIM1 INT,DIM2 INT,STARTTIME VARCHAR(30),ENDTIME VARCHAR(30),
...
DECLARE @TAB3 TABLE(EMPNODE BIGINT) 
		...
		BEGIN
			...
			BEGIN
				...
				SET @strQuery=@strQuery + 'INSERT INTO #TAB  
											...
											WHERE a.ISGROUP=0 AND a.NODEID<>1 AND C51.StatusID=250
	...
	SET @EMPLOYEEID=@EmpNode
		...
		DECLARE @STRHDCDIMENSIONS VARCHAR(MAX),@P INT,@PTR INT,@ColumnJoin NVARCHAR(MAX),@DefColumnJoin NVARCHAR(MAX),@ControlType Varchar(2),@HISTORYCCID BIGINT,@DIMNAME NVARCHAR(MAX)
		CREATE TABLE #HoursChart (ID BIGINT IDENTITY(1,1),NORMALHRS FLOAT,OT1HRS FLOAT,OT2HRS FLOAT,OT3HRS FLOAT,OT4HRS FLOAT,OT5HRS FLOAT)
