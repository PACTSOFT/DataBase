...
CREATE PROCEDURE [dbo].[spDOC_GetDocsUsedBudget]
	...
	@FromDate [datetime],
	@CCXML [nvarchar](500),
	@LinkedInvDocDetailsID [bigint] = 0,
...
SET NOCOUNT ON
	...
	DECLARE @XML XML,@ProductID BIGINT,@AccountID BIGINT,@SQL NVARCHAR(MAX),@CCWhere NVARCHAR(MAX),@Where NVARCHAR(MAX),@isinv bit,@dtype int,@AccountTypes nvarchar(max)
	DECLARE @tblCC AS TABLE(ID INT identity(1,1),CostCenterID INT,NodeID BIGINT)
	CREATE TABLE #tblMapAccs(ID BIGINT)
	...
	,@NonAccDocs nvarchar(MAX),@NonAccDocsField nvarchar(50),@InvAccDocs nvarchar(MAX),@InvAccDocsField nvarchar(50)
	...
	SELECT X.value('@CCID','INT'),X.value('@NodeID','BIGINT')
...
INNER JOIN COM_DocCCData DCC with(nolock) ON DCC.AccDocDetailsID=D.AccDocDetailsID'+@CCWhere+@From1+'
...
'+@DateField+'>=@From AND '+@DateField+'<=@To'+@AccWhere2+@Where+@StatusWhere
	...
	where D.CostCenterID IN ('+@InvAccDocs+') and '+@DateField+'>=@From AND '+@DateField+'<=@To'+@AccWhere2+@Where+@StatusWhere
		...
		begin
			...
			if exists (select * from #TblLinkDocs WITH(NOLOCK))
				SET @SQL=@SQL+'
	UNION ALL
	SELECT DocDate,T.DebitAmount*(Quantity-(Executed+Rejected))/Quantity,T.CreditAmount*(Quantity-(Executed+Rejected))/Quantity FROM (
	...
	where INV.CostCenterID IN ('+@NonAccDocs+') and '+@DateField+'>=@From AND '+@DateField+'<=@To'+replace(@AccWhere2,'D.','INV.')+replace(@Where,'D.','INV.')+@StatusWhere
			...
			if @LinkedInvDocDetailsID>0 AND @DocID=0 
				SET @SQL=@SQL+'
	UNION ALL
	SELECT DocDate,T.DebitAmount-(T.DebitAmount*(Quantity-Executed)/Quantity),T.CreditAmount-(T.CreditAmount*(Quantity-Executed)/Quantity) FROM (
	...
	where INV.InvDocDetailsID='+CONVERT(NVARCHAR,@LinkedInvDocDetailsID)+' and INV.CostCenterID IN ('+@NonAccDocs+') and '+@DateField+'>=@From AND '+@DateField+'<=@To'+replace(@AccWhere2,'D.','INV.')+replace(@Where,'D.','INV.')+@StatusWhere+'
			...
			BEGIN
				...
				SET @SQL=@SQL+'
	UNION ALL
	SELECT DocDate,T.DebitAmount*(Quantity-Executed)/Quantity,T.CreditAmount*(Quantity-Executed)/Quantity FROM (
