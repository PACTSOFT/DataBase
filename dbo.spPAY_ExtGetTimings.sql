...
BEGIN
	...
	CREATE TABLE #GENERALTIMINGS (ID BIGINT IDENTITY(1,1),STARTTIME TIME,ENDTIME TIME,LOCATION INT)
	...
	SET @STRQUERY=''
	SET @STRQUERY='INSERT INTO #GENERALTIMINGS
					 SELECT TOP 1 CAST(ISNULL(TD.DCALPHA1,''00:00'')AS TIME),CAST(ISNULL(TD.DCALPHA2,''00:00'')AS TIME),ISNULL(DC.dcCCNID2,0) FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DocCCData DC WITH(NOLOCK) ON ID.INVDOCDETAILSID=DC.INVDOCDETAILSID JOIN COM_DocTextData TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID
					 WHERE ID.COSTCENTERID=40052 AND ID.STATUSID=369 AND ISDATE(TD.dcAlpha3)=1 AND CONVERT(DATETIME,TD.dcAlpha3)<='''+ CONVERT(NVARCHAR,CONVERT(DATETIME,@DailyAttDate),106)+''''
