...
BEGIN
SET NOCOUNT ON;
DECLARE @REFDOCDETAILSD BIGINT,@LINKSTATUSID INT,@INVDOCDETAILSID BIGINT,@LINKEDINVDOCDETAILSID BIGINT,@LINKEDFIELDNAME NVARCHAR(100),@LINKEDFIELDVALUE FLOAT,@REFNO NVARCHAR(100)
DECLARE @TOTLOANAMOUNT FLOAT,@OPBAMOUNT FLOAT,@INSTAMOUNT FLOAT,@DEDMONTH DATETIME,@EMPNODE INT,@LOANID INT,@DOCDATE DATETIME
DECLARE @R INT,@TRC INT,@I INT,@DOCUMENTID INT
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				set @strQuery=@strQuery+' dcCurrID1=1,dcExchRT1=1'
				set @strQuery=@strQuery+' ,dcCalcNumFC1=N'''+ CONVERT(VARCHAR,@J)  +''','
				set @strQuery=@strQuery+' dcCalcNum1=N'''+ CONVERT(VARCHAR,@J)  +''','
				set @strQuery=@strQuery+' dcNum2=N'''+ CONVERT(VARCHAR,@INSTAMOUNT) +''','
				set @strQuery=@strQuery+' dcCurrID2=1,dcExchRT2=1'
				set @strQuery=@strQuery+' ,dcCalcNumFC2=N'''+ CONVERT(VARCHAR,@INSTAMOUNT)  +''','
				set @strQuery=@strQuery+' dcCalcNum2=N'''+ CONVERT(VARCHAR,@INSTAMOUNT)  +''','
				set @strQuery=@strQuery+' dcNum3=N'''+ CONVERT(VARCHAR,@INSTAMOUNT) +''','
				set @strQuery=@strQuery+' dcCurrID3=1,dcExchRT3=1'
				set @strQuery=@strQuery+' ,dcCalcNumFC3=N'''+ CONVERT(VARCHAR,@INSTAMOUNT)  +''','
			...
			END
				PRINT (@strQuery)
				set @strResult=''
				EXEC @strResult=spDOC_SetTempInvDoc 40057,0,'','',@DOCDATE,'','',@strQuery ,'','','','','false',0,0,0,1,'',0,0,'admin','admin',1,1,False
				IF(ISNULL(@strResult,'')<>'')
				BEGIN
					UPDATE INV_DOCDETAILS SET STATUSID=369,REFCCID=300,REFNODEID=@INVDOCDETAILSID,LINKSTATUSID=369 WHERE DOCID=CONVERT(BIGINT,@strResult) AND COSTCENTERID=40057
				END
		END
	SET @R=@R+1	
	END
