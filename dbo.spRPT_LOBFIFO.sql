...
BEGIN TRY  
SET NOCOUNT ON;  
	DECLARE @SQL NVARCHAR(MAX),@PDCSQL NVARCHAR(MAX),@DocDateCol nvarchar(50),@TagCol nvarchar(20),@TagDbCol nvarchar(20),@StrTemp nvarchar(max)
...
begin
	...
	set @str='	SELECT A.AccountID,' + @strCols + 'CONVERT(DATETIME, max(B.'+@DateFilter+')) DocDate,CONVERT(DATETIME,max(B.DueDate)) DueDate,CONVERT(DATETIME,max(B.DocDate)) VoucherDate,B.VoucherNo DocNo,
sum(' + @AmtColumn + ') Amount,sum(' + @AmtColumn + ') BillAmount'
	...
	set @str=@str+',C50002.Name AS Location'+@SELECTQUERY+',(max(B.DueDate)-max(B.DocDate)) CreditDays,@To-max(B.'+@DateFilter+') OverDue
...
--create table #tblAdj(ID int identity(1,1),AccountID bigint,Amount float)
...
--create table #tblLOB(x int identity(1,1))
	...
	begin
		set @SQL=@SQL+' 
	create table #tblAcc(GTID bigint Primary Key)
	insert into #tblAcc
...
create table #tblAdj(ID int identity(1,1),AccountID bigint,Amount float'
	...
	set @SQL=@SQL+')
create table #tblLOB(ID int identity(1,1) primary key,AccountID bigint'
	if(@TagID>0)
		set @SQL=@SQL+',dcCCNID'+(convert(nvarchar,@TagID-50000))
...
'
if @IsReceivable=0
set @SQL=@SQL+'update #tblLOB set AdjAmount=abs(AdjAmount)
'
	set @SQL=@SQL+'select AccountID,sum(Amount) Amount,sum(UnAdjustedAmount) UnAdjustedAmount from (
	select AccountID,sum(AdjAmount) Amount,0 UnAdjustedAmount from #tblLOB group by AccountID
	union all
	select AccountID,0,-sum(Amount) UnAdjustedAmount from #tblAdj group by AccountID) as T group by AccountID'
end


set @SQL=@SQL+'

declare @i int,@Cnt int,@AccountID bigint,@Amount float,@ID int,@PrevAccID bigint,@Bal float,@TAG int,@PrevTAG bigint
select @PrevAccID=0,@i=1,@Cnt=count(*) from #tblLOB
while(@i<=@Cnt)
begin'
	if(@TagID>0)
	begin
		set @SQL=@SQL+'
	select @AccountID=AccountID,@TAG='+@TagColID+'@Amount='+@Var1+' from #tblLOB where ID=@i
	if(@PrevAccID!=@AccountID or @TAG!=@PrevTAG)
	begin
		set @Bal=0
		set @PrevAccID=@AccountID
		set @PrevTAG=@TAG
		select @Bal='+@Var2+' from #tblAdj where AccountID=@AccountID and TAG=@TAG
	end
	'
	end
	else
	begin
		set @SQL=@SQL+'
	select @AccountID=AccountID,@Amount='+@Var1+' from #tblLOB where ID=@i
	if(@PrevAccID!=@AccountID)
	begin
		set @Bal=0
		set @PrevAccID=@AccountID
		select @Bal='+@Var2+' from #tblAdj where AccountID=@AccountID
	end
	'
	end
...
set @SQL=@SQL+'
select * from #tblLOB where Show=1
drop table #tblLOB
drop table #tblAdj
'
