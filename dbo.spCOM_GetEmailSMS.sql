...
BEGIN  
	...
	begin
		...
		inner join COM_Location L with(nolock) on L.NodeID=X.value('@LID','bigint')
		order by Name
...
BEGIN
	...
	begin
		...
		N.[From],N.DisplayName,N.[To],N.CC,N.BCC,N.IgnoreMailsTo,N.AttachmentType,N.AttachmentID,N.Subject,N.Body,N.Query,N.ExtendedQuery,N.FieldsXML,N.UIXML,N.SendReportAs,N.UserWiseDims
		,U.Email1 Login_Email1,U.UserName Login_UserName,U.FirstName Login_FirstName,U.MiddleName Login_MiddleName,U.LastName Login_LastName
		...
		E.[From],E.DisplayName,E.[To],E.CC,E.BCC,N.IgnoreMailsTo,E.AttachmentType,E.AttachmentID,E.Subject,E.Body,N.Query Query,N.ExtendedQuery ExtendedQuery,N.FieldsXML,N.UIXML,0 SendReportAs,N.UserWiseDims
		,U.Email1 Login_Email1,U.UserName Login_UserName,U.FirstName Login_FirstName,U.MiddleName Login_MiddleName,U.LastName Login_LastName
		FROM COM_SchEvents E WITH(nolock)
		...
		select X.value('@LID','bigint') LID,X.value('@DName','nvarchar(200)') DName,X.value('@Email','nvarchar(200)') Email,X.value('@Pwd','nvarchar(200)') Pwd
		...
		from @XML.nodes('/XML/Row') as Data(X)  
		inner join @Tbl T on T.LID=X.value('@LID','bigint')
	end
	else
	begin
		SELECT E.SchEventID EventID,E.CostCenterID,E.NodeID,E.GUID EventGUID,E.SubCostCenterID,E.SubNodeID,E.TemplateID,N.TemplateType,E.OtherDocsNos,E.FilterXML,
...
BEGIN  
	declare @DATA xml,@Type int,@ReportID bigint
	set @DATA=@Param1
	SELECT @Type=X.value('@Type','int'),@ReportID=X.value('@ReportID','bigint') from @DATA.nodes('XML') as DATA(X)
	SELECT TemplateID,TemplateName FROM COM_NotifTemplate WITH(NOLOCK) 
