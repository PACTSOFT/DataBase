...
SET NOCOUNT ON;  
		...
		Declare @i int,@cnt int,@ii int,@ccnt int,@DocID bigint,@DELETECCID int,@return_value int,@VoucherNo nvarchar(200),@DocPrefix nvarchar(200),@dt datetime,@DocNumber nvarchar(50),@CCCols nvarchar(max),@sql nvarchar(max)
		declare @xml xml,@DocAbbr nvarchar(200),@DocumentTypeID bigint,@DocumentType int,@DocOrder int,@creatdt float,@Credid bigint,@DebitAcc bigint,@amt float,@acc bigint,@qry nvarchar(max),@GUID nvarchar(max)
		declare @ttdel table(id int identity(1,1),accid bigint,dt datetime,data nvarchar(max))
		declare @tbltran table(id int identity(1,1),CredAcc bigint,DebitAcc bigint,amt float,Qry nvarchar(max))
		...
		BEGIN
			...
			delete from @tbltran
			insert into @tbltran	
			select X.value('@CredAcc','bigint'),X.value('@DebitAcc','bigint'),X.value('@Amount','float'),X.value('@Query','nvarchar(max)') 
