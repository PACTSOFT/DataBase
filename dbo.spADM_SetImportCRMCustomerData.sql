...
GO
CREATE PROCEDURE [dbo].[spADM_SetImportCRMCustomerData]
	@XML [nvarchar](max),
...
SET NOCOUNT ON	
      ...
      --     ,X.value('@AccountName','nvarchar(max)') from @DATA.nodes('/XML/Row') as Data(X)
		...
		begin try
			...
			begin
 				if @VehicleID is not null and @VehicleID>0 and  @IsCode is not null and @IsCode=0 and exists(select ProductID from dbo.INV_Product  with(nolock) where ProductName=@AccountName)
				begin
				...
				if not exists (select ProductID from SVC_ProductVehicle where ProductID=@NodeID and VehicleID=@VehicleID)
					begin
				...
				else if @VehicleID is not null and @VehicleID>0 and  @IsCode is not null and @IsCode=1 and exists(select ProductID from dbo.INV_Product  with(nolock) where ProductCode=@AccountCode)
				begin
		...
		END 
			...
			BEGIN	
				...
				BEGIN
				if( @IsCode is not null and @IsCode =0)
					set @NodeID= (Select  top 1 CustomerID from SVC_Customers where CustomerName=@AccountName) 
				else
					set @NodeID= (Select  top 1 CustomerID from SVC_Customers where CustomerCode=@AccountCode) 
					--set @NodeID= (Select  top 1  CustomerID from SVC_Customers where CustomerName=@AccountName)
					set @AccountCode= (Select CustomerCode from SVC_Customers where CustomerID=@NodeID)
				END
				ELSE IF @COSTCENTERID=83
				BEGIN
						if( @IsCode is not null and @IsCode =0)
			...
			if (@Substitutes is not null and @Substitutes<>'')
			 begin
				...
				ELSE
				BEGIN
						SELECT @SubGroupID=ISNULL(MAX(SubstituteGroupID),0)+1 FROM [INV_ProductSubstitutes] 
				END
				
				if   @IsCode is not null and @IsCode=1 and exists(select top 1  
				ProductID from dbo.INV_Product  with(nolock) where ProductCode=@AccountCode)
				begin
					set @NodeID=(select top 1 ProductID from dbo.INV_Product with(nolock) where ProductCode=@AccountCode) 
				end
				else
					...
					INSERT INTO [INV_ProductSubstitutes]  
						...
						VALUES (@SubGroupID, @SubGroupName, @NodeID,0,NEWID()
			...
			begin
				...
				--select @PartID=CCNID29 from INV_ProductCostCenterMap with(nolock) where ProductID=@NodeID
				...
				begin
					/*update INV_ProductCostCenterMap
					set INV_ProductCostCenterMap.CCNID37=a.CCNID37,INV_ProductCostCenterMap.CCNID30=a.CCNID30,INV_ProductCostCenterMap.CCNID6=a.CCNID6
					from dbo.COM_CCCCData a
					where a.NODEID=INV_ProductCostCenterMap.CCNID29 and a.COSTCENTERID=50029*/
					-- Need to check this CONDITION

					--update dbo.INV_Product
					--set INV_Product.CategoryID=a.CCNID6
					--from dbo.INV_ProductCostCenterMap a
					--where a.ProductID=INV_Product.ProductID	
