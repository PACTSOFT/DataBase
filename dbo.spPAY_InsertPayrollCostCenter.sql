...
BEGIN
SET NOCOUNT ON;
DECLARE @DOCID INT, @INVDOCDETAILSID BIGINT,@INVDOCDETAILSIDLAST BIGINT,@DOCSEQNO INT,@LEAVES FLOAT,@DOCUMENTID BIGINT
DECLARE @ALSTARTMONTH INT,@ALStartMonthYear DATETIME,@ALEndMonthYear DATETIME,@Date DATETIME
DECLARE @EMP INT,@GRADE INT,@LEAVETYPE BIGINT,@EMPGRADE INT,@I INT,@TRC INT,@MonthNo INT,@ACTUALLEAVES FLOAT,@DECVAL FLOAT,@J INT,@RC INT,@FROMDATE DATETIME
CREATE TABLE #TAB(ID INT IDENTITY(1,1),dcID INT,dcCCNID51_Key BIGINT,dcCCNID53_Key BIGINT,dcCCNID52_Key BIGINT,
				   PrevYearAlloted BIGINT,dcNum3 float,PrevYearBalanceOB INT,CurrYearConsumed DECIMAL(9,2),Balance DECIMAL(9,2))	
CREATE TABLE #TAB1 (ID INT IDENTITY(1,1),dcID INT,dcCCNID51_Key BIGINT,dcCCNID53_Key BIGINT,dcCCNID52_Key BIGINT,
				   PrevYearAlloted BIGINT,dcNum3 float,PrevYearBalanceOB INT,CurrYearConsumed DECIMAL(9,2),Balance DECIMAL(9,2))	
DECLARE @TABDOCID TABLE(ID BIGINT IDENTITY(1,1),DOCID BIGINT,FROMDATE DATETIME)					

IF(@CostCenterID=50051)
	SET @Date=(SELECT CONVERT(DATETIME,DOJ) FROM COM_CC50051 WITH(NOLOCK) WHERE NODEID=@NodeID)
...
--	BEGIN
		INSERT INTO @TABDOCID
				SELECT DISTINCT DOCID,CONVERT(DATETIME,TD.DCALPHA3) FROM INV_DOCDETAILS ID WITH(NOLOCK),COM_DOCTEXTDATA TD WITH(NOLOCK) WHERE ID.INVDOCDETAILSID=TD.INVDOCDETAILSID AND ID.COSTCENTERID=40081 AND ISDATE(TD.DCALPHA3)=1 AND YEAR(CONVERT(DATETIME,TD.DCALPHA3))>=YEAR(CONVERT(DATETIME,@ALSTARTMONTHYEAR))
		...
		BEGIN	
			...
			IF((SELECT COUNT(*) FROM  INV_DOCDETAILS ID WITH(NOLOCK),COM_DOCCCDATA CC WITH(NOLOCK)
			...
			BEGIN
				...
				inner join COM_CCCCData cc on cc.nodeid=c51.nodeid and cc.costcenterid=50051 and isnull(cc.ccnid53,'')<>''
