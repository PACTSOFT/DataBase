...
SET NOCOUNT ON 
	...
	Declare @CCID BIGINT,@StageCCID BIGINT,@Sql nvarchar(max),@tableName nvarchar(200),@DocumentType int
	declare @VType int,@MachineDim BIGINT,@DbCol nvarchar(500),@CrCol nvarchar(500),@DrAccID BIGINT,@CrAccID BIGINT
	

	select @DocumentType=DocumentType from ADM_DocumentTypes where CostCenterID=@CostcenterID
	...
	select @StageCCID=Value from COM_CostCenterPreferences 
	...
	,a.CCID,a.CCNodeID 
	from  PRD_BillOfMaterial a
	join PRD_BOMProducts b on a.[BomID]=b.[BOMID] and b.ProductUse=2
	join INV_Product i on a.ProductID=i.ProductID
	where a.BOMID=@BomID
	...
	BEGIN
		...
		FROM  [PRD_Expenses] BE with(nolock)
		 join PRD_BOMStages s on BE.[StageID]=s.[StageID]
	...
	BEGIN		
		...
		begin
			...
			join ADM_DocumentDef b on a.CostCenterColID=b.CostCenterColID
			left join ADM_CostCenterDef c on c.CostCenterColID=b.CrRefColID
			left join ADM_CostCenterDef d on d.CostCenterColID=b.DrRefColID
			...
			join sys.tables b on a.object_id=b.object_id
			where a.name=@CrCol and b.name=@tableName)
				set @Sql=@Sql +' left join acc_accounts cr on cr.accountID=D.'+@CrCol
			Else if(@CrAccID>0)
				set @Sql=@Sql +' left join acc_accounts cr on cr.accountID='+CONVERT(nvarchar,@CrAccID)
			if exists(select a.name from sys.columns a
			join sys.tables b on a.object_id=b.object_id
			where a.name=@DbCol and b.name=@tableName)
				set @Sql=@Sql +' left join acc_accounts Dr on Dr.accountID=D.'+@DbCol
			Else if(@DrAccID>0)
				set @Sql=@Sql +' left join acc_accounts Dr on Dr.accountID='+CONVERT(nvarchar,@DrAccID)

			 if(@StageCCID>0)
			  set @Sql=@Sql +' join PRD_BOMStages s on BR.[StageID]=s.[StageID]'
		  ...
		  ,a.AutoStage,[UnitPrice],[ExchgRT],a.[CurrencyID],[Value],a.[Wastage],[FilterOn],a.[StageID],U.BaseName,U.UnitName,U.Conversion
		   ...
		   ,0 as Used '
			...
			left join COM_UOM U WITH(NOLOCK) on a.UOMID=U.UOMID '
			 if(@StageCCID>0)
			  set @Sql=@Sql +' join PRD_BOMStages s on a.[StageID]=s.[StageID]'
	...
	from PRD_BillOfMaterial a
	join PRD_BillOfMaterialExtended b on a.BOMID=b.BOMID
	Where a.BOMID=@BomID
	...
	BEGIN
		...
		join '+@tableName+' b on a.StageNodeID=b.NodeID
