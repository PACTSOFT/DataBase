...
BEGIN
	...
	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PAY_DocNumData]'))
	BEGIN
		DECLARE @SQL NVARCHAR(MAX)
		SET @SQL='TRUNCATE TABLE PAY_DocNumData'
		EXEC(@SQL)
	END
	TRUNCATE TABLE PAY_DocNumData
...
--PRODUCTION
	...
	DELETE FROM PRD_BillOfMaterial WHERE BOMID in (select BOMID FROM PRD_BillOfMaterial WHERE BOMCode<>'BOMCode')
	UPDATE PRD_BillOfMaterial SET RGT=2
	SELECT @MAXVALUE=BOMID FROM PRD_BillOfMaterial WHERE BOMCode='BOMCode'
...
--SERVICE
	TRUNCATE TABLE SVC_ServiceTicketHistory
	TRUNCATE TABLE SVC_ServiceTicketDatesComm
	TRUNCATE TABLE SVC_ServiceDetailsHistory
	TRUNCATE TABLE SVC_ServicePartsInfoHistory
	TRUNCATE TABLE SVC_ServicesReasons
	TRUNCATE TABLE SVC_ServiceTicketTaxes
	TRUNCATE TABLE SVC_InsuranceDetails
	TRUNCATE TABLE SVC_ServiceCostCenterMap
	--TRUNCATE TABLE SVC_CostCenter
	TRUNCATE TABLE SVC_ShopSupplies
	TRUNCATE TABLE SVC_ServiceTicketBillPayment
	TRUNCATE TABLE SVC_ServiceTicketIndent
	TRUNCATE TABLE SVC_Appointment
	TRUNCATE TABLE SVC_ServiceJobProcess
	TRUNCATE TABLE SVC_BreakDownTicket
	--TRUNCATE TABLE SVC_ServiceTypes
	DELETE FROM SVC_ServiceTypes
	DBCC CHECKIDENT(SVC_ServiceTypes,RESEED,0)	
	
	TRUNCATE TABLE SVC_ServiceTicketClaims
	TRUNCATE TABLE SVC_ServiceJobsInfo
	TRUNCATE TABLE SVC_ServicePartsInfo
	TRUNCATE TABLE SVC_LocationTaxes
	
	
	DELETE FROM SVC_BreakDownTicketBillPayment
	DBCC CHECKIDENT(SVC_BreakDownTicketBillPayment,RESEED,0)	
	--TRUNCATE TABLE SVC_BreakDownTicketBillPayment
	TRUNCATE TABLE SVC_ProductVehicle
	TRUNCATE TABLE SVC_ServiceJobsInfoHistory
	TRUNCATE TABLE SVC_ServiceTicketBillHistory
	TRUNCATE TABLE SVC_PackageParts
	--TRUNCATE TABLE SVC_CustomerTypes
	TRUNCATE TABLE SVC_ServiceTicketBill
	TRUNCATE TABLE SVC_FollowUpTypes
	TRUNCATE TABLE SVC_VehicleDuplicate
	TRUNCATE TABLE SVC_Package
	TRUNCATE TABLE SVC_ServiceDetailsOptions
	TRUNCATE TABLE SVC_Feedback
	TRUNCATE TABLE SVC_PartCategoryMap
	TRUNCATE TABLE SVC_PartVehicle
	TRUNCATE TABLE SVC_PartCategory
	TRUNCATE TABLE SVC_PackagesCCMap
	
	--TRUNCATE TABLE SVC_ServiceTicket
	DELETE FROM SVC_ServiceTicket
	DBCC CHECKIDENT(SVC_ServiceTicket,RESEED,0)	
	
	TRUNCATE TABLE SVC_VehicleCheckout
	TRUNCATE TABLE SVC_PriceMargin
	TRUNCATE TABLE SVC_TicketDiscPreferences
	TRUNCATE TABLE SVC_ServiceTicketFollowUp
	TRUNCATE TABLE SVC_ServiceDetails
	TRUNCATE TABLE SVC_AppointmentServiceDetails
	TRUNCATE TABLE SVC_CustomerFamilyDetails
	--TRUNCATE TABLE SVC_CustomersVehicle
	DELETE FROM SVC_CustomersVehicle
	DBCC CHECKIDENT(SVC_CustomersVehicle,RESEED,0)
	TRUNCATE TABLE SVC_CustomersVehicleExtended
	TRUNCATE TABLE SVC_CustomersExtended
	TRUNCATE TABLE SVC_CustomerCostCenterMap
	TRUNCATE TABLE SVC_ServiceTicketDiscDetails
	
	DELETE FROM SVC_Customers WHERE CustomerID>1
	UPDATE SVC_Customers SET LFT=1,RGT=2 WHERE CustomerID=1
	DBCC CHECKIDENT(SVC_Customers,RESEED,1)
	
	DELETE FROM SVC_Vehicle	WHERE VehicleID>0
	DBCC CHECKIDENT(SVC_Vehicle,RESEED,0)
	
--RENTAL
	TRUNCATE TABLE REN_CollectionHistory
...
--CRM
	Truncate Table CRM_CampaignInvites
	...
	Truncate Table CRM_OpportunityProductsExtended
...
--DIMENSION TABLES
	...
	DECLARE @I INT, @CCID INT,@CCNAME NVARCHAR(50),@STRQRY NVARCHAR(MAX)
	SET @I=9
	SET @CCNAME=''
	SET @STRQRY=''
	WHILE(@I<=70)
	BEGIN
		SET @CCID=50000+@I
		IF(@CCID NOT IN (50052,50058,50059,50060,50061,50062,50063,50064,50065,50066,50067,50070,50071,50072) )
		BEGIN
			SET @CCNAME='COM_CC'+CONVERT(NVARCHAR,@CCID)
			PRINT @CCNAME
			IF(@CCID=50054)
			BEGIN
				SET @STRQRY='DELETE FROM ' + CONVERT(VARCHAR,@CCNAME)+''
				--PRINT (@STRQRY)
				EXEC (@STRQRY)
			...
			ELSE
			BEGIN
			if exists (select * from information_schema.tables where table_name=@CCNAME)
			begin
				SET @STRQRY='DELETE FROM ' + CONVERT(VARCHAR,@CCNAME)+'	WHERE NODEID>2'
				SET @STRQRY=@STRQRY+' UPDATE ' + CONVERT(VARCHAR,@CCNAME)+' SET lft=2,RGT=3 WHERE NODEID=1'
				SET @STRQRY=@STRQRY+' UPDATE ' + CONVERT(VARCHAR,@CCNAME)+' SET lft=1,RGT=4 WHERE NODEID=2'
				--PRINT (@STRQRY)
				EXEC (@STRQRY)
				DBCC CHECKIDENT(@CCNAME,RESEED,4)	
			end
			END
		END
	SET @I=@I+1	
	...
	Delete From	COM_Division where NODEID>2
	UPDATE COM_Division SET lft=2,RGT=3 WHERE NODEID=1
	UPDATE COM_Division SET lft=1,RGT=4 WHERE NODEID=2
	DBCC CHECKIDENT(COM_Division,RESEED,2)	
	
	Delete From	COM_Location where NODEID>2
	UPDATE COM_Location SET lft=2,RGT=3 WHERE NODEID=1
	UPDATE COM_Location SET lft=1,RGT=4 WHERE NODEID=2
	DBCC CHECKIDENT(COM_Location,RESEED,2)	
	
	Delete From	COM_Branch where NODEID>2
	UPDATE COM_Branch SET lft=2,RGT=3 WHERE NODEID=1
	UPDATE COM_Branch SET lft=1,RGT=4 WHERE NODEID=2
	DBCC CHECKIDENT(COM_Branch,RESEED,2)	
	
	Delete From	COM_Department where NODEID>2
	UPDATE COM_Department SET lft=2,RGT=3 WHERE NODEID=1
	UPDATE COM_Department SET lft=1,RGT=4 WHERE NODEID=2
	DBCC CHECKIDENT(COM_Department,RESEED,2)	
	
	Delete From	COM_Category where NODEID>2
	UPDATE COM_Category SET lft=2,RGT=3 WHERE NODEID=1
	UPDATE COM_Category SET lft=1,RGT=4 WHERE NODEID=2
	DBCC CHECKIDENT(COM_Category,RESEED,2)	
	
	Delete From	COM_Area where NODEID>2
	UPDATE COM_Area SET lft=2,RGT=3 WHERE NODEID=1
	UPDATE COM_Area SET lft=1,RGT=4 WHERE NODEID=2
	DBCC CHECKIDENT(COM_Area,RESEED,2)	
	
	Delete From	COM_Salesman where NODEID>2
	UPDATE COM_Salesman SET lft=2,RGT=3 WHERE NODEID=1
	UPDATE COM_Salesman SET lft=1,RGT=4 WHERE NODEID=2
	DBCC CHECKIDENT(COM_Salesman,RESEED,2)	
	
	Delete From	COM_Teritory where NODEID>2
	UPDATE COM_Teritory SET lft=2,RGT=3 WHERE NODEID=1
	UPDATE COM_Teritory SET lft=1,RGT=4 WHERE NODEID=2
	DBCC CHECKIDENT(COM_Teritory,RESEED,2)	
