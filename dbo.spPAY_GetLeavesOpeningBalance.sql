...
SET @SQL='INSERT INTO #TABALOP
	...
	WHERE a.StatusID=369 AND (A.CostCenterID=40081 OR (CostcenterID=40060 AND (CommonNarration=''#Opening#'' OR CommonNarration=''#CarryForward#''))) AND b.dcCCNID51 IN('+@EmpIDs+') AND ISDATE(TD.dcAlpha3)=1 
	...
	BEGIN
		...
		IF @IsGradeWiseMP=1
		BEGIN
			SELECT @Payrolltype=ISNULL(CARRYFORWARD,''),@MaxCarryForwardDays=ISNULL(MaxCarryForwardDays,0) FROM COM_CC50054 WITH(NOLOCK) WHERE GRADEID=@Grade AND COMPONENTID=@LeaveType AND PAYROLLDATE=(SELECT MAX(PAYROLLDATE) FROM COM_CC50054  WITH(NOLOCK) WHERE CONVERT(DATETIME,PAYROLLDATE)<=CONVERT(DATETIME,@PayrollDate) AND GradeID=@Grade)
		END
		ELSE
		BEGIN
			SELECT @Payrolltype=ISNULL(CARRYFORWARD,''),@MaxCarryForwardDays=ISNULL(MaxCarryForwardDays,0) FROM COM_CC50054 WITH(NOLOCK) WHERE GRADEID=1 AND COMPONENTID=@LeaveType AND PAYROLLDATE=(SELECT MAX(PAYROLLDATE) FROM COM_CC50054  WITH(NOLOCK) WHERE CONVERT(DATETIME,PAYROLLDATE)<=CONVERT(DATETIME,@PayrollDate) AND GradeID=1)
		END
		...
		EXEC spPAY_GetCurrYearLeavesInfo @FDYear,@TDYear,@EmployeeID,@LeaveType,@Userid,@Langid,@Date,0,@PrevYearLeavestaken OUTPUT,@PrevYearNOOFHOLIDAYS OUTPUT,@PrevYearWEEKLYOFFCOUNT OUTPUT,@PrevYearExstAppliedEncashdays OUTPUT
			...
			WHERE  ISDATE(TD.DCALPHA3)=1 AND ISDATE(TD.DCALPHA4)=1 AND ISDATE(TD.dcAlpha3)=1 AND CONVERT(DATETIME,TD.dcAlpha3) between CONVERT(DATETIME,CONVERT(NVARCHAR,@FDYear)) and CONVERT(DATETIME,CONVERT(NVARCHAR,@TDYear)) 
				   AND CC.DCCCNID51=@EmployeeID AND CC.DCCCNID52=@LeaveType AND (ID.COSTCENTERID=40081 OR (CostcenterID=40060 AND (ID.CommonNarration='#Opening#' OR ID.CommonNarration='#CarryForward#'))) AND ID.STATUSID=369
