...
BEGIN TRY  
   ...
   declare @ErrorMsg nvarchar(max)
	...
	BEGIN--CREATE Case 
		...
		INSERT INTO [REN_Units]([PropertyID],[Code],[Name],[Status],[Depth],[ParentID],[lft],[rgt],[IsGroup],[CompanyGUID],[GUID],[CreatedBy],[CreatedDate],CCID,CCNodeID,LinkCCID,CodePrefix,CodeNumber,GroupSeqNoLength)    
		VALUES (@PROPERTYID,@CODE,@NAME,@STATUSID,@Depth,@SelectedNodeID,@lft,@rgt,@IsGroup,@CompanyGUID,newid(),@UserName,convert(float,@Dt),@CCID,@LinkDim_NodeID,@Dimesion,@CodePrefix,@CodeNumber,@GroupSeqNoLength)    
    ...
    begin
		...
		insert into REN_Particulars(ParticularID,[PropertyID]    
			   ...
			   ,[GUID]    
			   ,[CreatedBy]    
			   ,[CreatedDate],DimNodeID,Display,BankAccountID)    
