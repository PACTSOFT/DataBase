...
JOIN INV_DOCDETAILS ID WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID
...
SET @DAILYATTENDANCEDATE=(SELECT TOP 1 CONVERT(DATETIME,DAILYATTDATE) FROM @TAB1)
	...
	BEGIN	
			...
			UPDATE T1 SET T1.PREVIOUSRECORDENDDATIME=CONVERT(DATETIME,T.EDT) FROM @TAB1 AS T1 INNER JOIN 
							(SELECT TOP 1 DC.DCCCNID51 A ,CONVERT(DATETIME,TD.DCALPHA3) EDT FROM COM_DOCTEXTDATA TD WITH(NOLOCK)
								JOIN INV_DOCDETAILS ID WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID
								JOIN COM_DOCCCDATA DC WITH(NOLOCK) ON ID.INVDOCDETAILSID=DC.INVDOCDETAILSID
