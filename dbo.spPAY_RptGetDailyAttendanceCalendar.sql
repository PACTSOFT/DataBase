...
SELECT DCALPHA1,DCALPHA3,DCCCNID51,dcAlpha2 INTO #T1 FROM INV_DOCDETAILS ID 
...
BEGIN
	...
	SELECT @StartTime=SUBSTRING(CONVERT(VARCHAR(8), CONVERT(DATETIME,dcAlpha2), 108),1,5),@EndTime=SUBSTRING(CONVERT(VARCHAR(8), CONVERT(DATETIME,dcAlpha3), 108),1,5) FROM #T1		WHERE DCCCNID51=@NODEID AND (ISDATE(DCALPHA1)=1 AND CONVERT(DATETIME,DCALPHA1)=CONVERT(DATETIME,@DT1))
