...
SET NOCOUNT ON
	...
	((FEATUREID > 50000 AND FEATUREID <= 50050) OR FEATUREID IN (2,3,51,57,58,300,65,71,76,72,80,84,81,86,83,88,78,73,89,82,16,92,93))
	...
	SELECT Name ,FeatureId,TableName FROM Adm_Features WHERE IsEnabled=1 AND (FeatureID BETWEEN 50001 AND 50050) ORDER BY FeatureID
        
    SELECT CostCenterID,DocumentName,CASE WHEN IsInventory=1 THEN 'True' ELSE 'False' END IsInventory,DocumentType FROM ADM_DocumentTypes ORDER BY DocumentName
	...
	SELECT UserColumnName CostCenterName,CONVERT(NVARCHAR,(50000+CONVERT(INT,REPLACE(SysColumnName,'CCNID','')))) CostCenterID FROM Adm_CostCenterDef WITH(NOLOCK) WHERE CostCenterId=50051 AND IscolumnInUse=1 AND SysColumnName LIKE 'CCNID%' ORDER BY UserColumnName
