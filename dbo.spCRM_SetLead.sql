...
BEGIN TRY  
 ...
 DECLARE @UpdateSql nvarchar(max),@Dt FLOAT, @TempGuid nvarchar(50),@HasAccess bit,@ActionType INT,@StatusName nvarchar(50),@ID BIGINT,@Remarks nvarchar(max)  
 Declare @XML XML,@return_value bigint,@LinkDim_NodeID int,@Dimesion INT,@RefSelectedNodeID BIGINT ,@CCStatusID bigint,@Sql nvarchar(max)  
 ...
 BEGIN--CREATE Lead    
  ...
  DECLARE @Str nvarchar(50), @a int, @val bigint,@cnt int, @DIMContact nvarchar(max),  
 ...
 BEGIN  
  ...
  Delete from CRM_FEEDBACK where CCNodeID = @LeadID and CCID=86  
	...
	EXEC sp_executesql @SQL,N'@XML XML,@LeadID BIGINT',@XML,@LeadID          
 END  
 ...
 BEGIN    
  ...
  INNER JOIN @XML.nodes('/NotesXML/Row') as Data(X)      
  ON convert(bigint,X.value('@NoteID','bigint'))=C.NoteID AND X.value('@Action','NVARCHAR(10)')='DELETE'  )
