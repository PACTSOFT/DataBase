...
DECLARE @FROMDATE DATETIME,@INVDOCID BIGINT,@EMPNODE INT,@STATUSID INT
DECLARE @ICOUNT INT,@RCOUNT INT,@LinkedInvDocDetailsID INT
DECLARE @Year INT,@ALStartMonth INT,@ALStartMonthYear DATETIME,@ALEndMonthYear DATETIME,@LEAVETYPEGP INT,@CURRYEARALDOCNO BIGINT

SET @ICOUNT=1

DECLARE @TAB1 TABLE (ID BIGINT IDENTITY(1,1),INVDOCDETAILSID BIGINT,FROMDATE DATETIME,EMPNODE INT)
DECLARE @UPDATEASSIGNLEAVES TABLE(InvDocID BIGINT,dcCCNID51 BIGINT,dcCCNID53 BIGINT,dcCCNID52 BIGINT,
	...
	BEGIN
		...
		BEGIN
				INSERT INTO @TAB1
				SELECT ID.INVDOCDETAILSID,CONVERT(DATETIME,TD.dcAlpha1),ISNULL(CC.DCCCNID51,0)
				...
				WHERE  ID.INVDOCDETAILSID=TD.INVDOCDETAILSID  AND ID.INVDOCDETAILSID=CC.INVDOCDETAILSID AND TD.INVDOCDETAILSID=CC.INVDOCDETAILSID
					...
					SELECT @RCOUNT =COUNT(*) FROM @TAB1
				  	WHILE(@ICOUNT<=@RCOUNT)
					BEGIN
								SELECT @INVDOCID=INVDOCDETAILSID,@FROMDATE=CONVERT(DATETIME,FROMDATE),@EMPNODE=EMPNODE FROM @TAB1 WHERE ID=@ICOUNT
