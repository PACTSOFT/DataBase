		...
		BEGIN
			...
			print @WhereCond
				SET @WhereCond=@WhereCond+' 
											AND b.dcCCNID'+CONVERT(NVARCHAR,(@HCCID-50000)) +' IN (SELECT 1 as CCNID UNION ALL Select CCNID'+CONVERT(NVARCHAR,(@HCCID-50000))+' FROM COM_CCCCData WITH(NOLOCK) WHERE CostCenterID=50051 AND NodeID IN('+CONVERT(NVARCHAR,@EmployeeID)+') ) '
			END
			ELSE
			BEGIN
				SET @WhereCond=@WhereCond+' AND b.dcCCNID'+CONVERT(NVARCHAR,(@HCCID-50000)) +' IN (SELECT 1 as HistoryNodeID UNION ALL SELECT DISTINCT HistoryNodeID FROM COM_HistoryDetails WITH(NOLOCK) 
