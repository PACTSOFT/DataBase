...
SET NOCOUNT ON    
	...
	set @sql='SELECT S.ScheduleID,CC.NodeID,D.DocNo VoucherNo,CONVERT(DATETIME, E.EventTime) AS EventTime, CONVERT(NVARCHAR, S.FreqType) AS FreqType,
					E.StatusID AS Status, CC.CostCenterID,E.SchEventID,E.SubCostCenterID,E.NODEID sno,AttachmentID,S.RecurMethod
					...
					where docid=D.ID and costcenterid=CC.CostCenterID and CanRecur=1) Amount
		...
		JOIN COM_DocID AS D on D.ID=CC.NodeID
		JOIN [REN_ContractDocMapping] AS DM on D.ID=DM.DocID
		join ren_contract a WITH(NOLOCK) on dm.[ContractID]=a.[ContractID] '
		if(@FROMQRY!='')
		  set @sql=@sql+' join COM_ccCCData L with(nolock) on a.ContractID=L.NodeID and l.CostCenterID=95 '+@FROMQRY
		...
		left JOIN COM_DocID AS PD on PD.DocNo=e.POstedvoucherno
		...
		set @sql=@sql+' ORDER BY EventTime DESC'
		print @sql
	exec(@sql)
