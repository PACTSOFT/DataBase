...
BEGIN			
		...
		,0,ISNULL(dcAlpha16,''),CONVERT(FLOAT,dcAlpha14),CONVERT(DATETIME,dcAlpha1) 
		FROM COM_DocTextData TD with(nolock),INV_DOCDETAILS ID with(nolock), COM_DocCCData DC with(nolock) 
		WHERE  LEN(dcAlpha2)<30 AND  LEN(dcAlpha3)<30 AND ISDATE(dcAlpha2)=1 AND  ISDATE(dcAlpha3)=1 AND TD.InvDocDetailsID=DC.InvDocDetailsID AND	ID.InvDocDetailsID=TD.InvDocDetailsID AND ID.CostCenterID=40072 AND DC.DCCCNID51=@EmployeeID  AND ID.STATUSID NOT IN (372,376)
		And (
	   ...
	   SET @IC=1
		...
		END	
		--UPDATE DATESCOUNT SET DATESCOUNT.count=4 FROM @TabDatesRange DATESCOUNT inner join COM_DocTextData TD on CONVERT(DateTime,DATESCOUNT.DATE1)=CONVERT(DateTime,TD.dcAlpha1)
		-- inner join INV_DOCDETAILS ID  on  ID.INVDOCDETAILSID=TD.INVDOCDETAILSID and ISDATE(TD.dcAlpha1)=1 AND 
		-- CONVERT(DateTime,DATE1) = CONVERT(DateTime,TD.dcAlpha1) AND ID.COSTCENTERID=40051
