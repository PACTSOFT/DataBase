...
BEGIN TRY   
	...
	BEGIN
		DECLARE @TAB TABLE(ID INT IDENTITY(1,1),LFT INT,	ORDERNO INT,			PARENTID INT,
					   ...
					   AMOUNTLIMIT VARCHAR(100),	AMOUNT decimal(9,2),	SELECTEDNODE INT, SELECTEDAMTLIMIT VARCHAR(100))
		...
		WHILE(@R<=@TRC)
			BEGIN
				INSERT INTO @TAB SELECT  LFT, ORDERNO,PARENTID,NODEID,NAME,ISGROUP,CONTROLTYPE ,FIELDTYPE,GROUPID ,AMOUNTLIMIT,AMOUNT,SELECTEDNODE,SELECTEDAMTLIMIT FROM @TABGROUPS WHERE ID=@R
		...
		BEGIN
			...
			SELECT LFT,ORDERNO,PARENTID,NODEID,NAME,ISGROUP,CONTROLTYPE ,FIELDTYPE,GROUPID ,AMOUNTLIMIT,AMOUNT,SELECTEDNODE,SELECTEDAMTLIMIT  FROM @TAB ORDER BY ID,PARENTID,NODEID,ORDERNO
		END
		...
		BEGIN
			...
			UPDATE T SET T.AMOUNTLIMIT=T1.AMOUNTLIMIT,T.SELECTEDAMTLIMIT=T1.AMOUNTLIMIT FROM @TAB T INNER JOIN PAY_YearwiseTaxAmountLimit T1 ON T.NODEID=T1.COMPONENTID AND T1.YEAR=@YEAR
	...
	BEGIN
		...
		IF ((SELECT COUNT(*) FROM PAY_YearwiseTaxAmountLimit WITH(NOLOCK) WHERE YEAR=@YEAR)>0)
			UPDATE T SET T.AMOUNTLIMIT=T1.AMOUNTLIMIT FROM @TAB1 T INNER JOIN PAY_YearwiseTaxAmountLimit T1 ON T.NODEID=T1.COMPONENTID AND T1.YEAR=@YEAR
		
		INSERT INTO @TAB1
			SELECT  @RGT+1,0,0,994,'Other Components',1,'Label','',0,0
		INSERT INTO @TAB1
			SELECT  @RGT+2,0,0,995,'80C Amount Limit',0,'TextBox','',0,(SELECT ISNULL(AmountLimit,0) FROM PAY_YearwiseTaxAmountLimit WHERE COMPONENTID=995 AND YEAR=@YEAR)
		INSERT INTO @TAB1
			SELECT  @RGT+3,0,0,996,'Per Children Amount',0,'TextBox','',0,(SELECT ISNULL(AmountLimit,0) FROM PAY_YearwiseTaxAmountLimit WHERE COMPONENTID=996 AND YEAR=@YEAR)
		INSERT INTO @TAB1
			SELECT  @RGT+4,0,0,997,'No Of Children',0,'TextBox','',0,(SELECT ISNULL(AmountLimit,0) FROM PAY_YearwiseTaxAmountLimit WHERE COMPONENTID=997 AND YEAR=@YEAR)
		INSERT INTO @TAB1
			SELECT  @RGT+5,0,0,998,'Conveyance',0,'TextBox','',0,(SELECT ISNULL(AmountLimit,0) FROM PAY_YearwiseTaxAmountLimit WHERE COMPONENTID=998 AND YEAR=@YEAR)
		INSERT INTO @TAB1
			SELECT  @RGT+6,0,0,999,'Rebate',0,'TextBox','',0,(SELECT ISNULL(AmountLimit,0) FROM PAY_YearwiseTaxAmountLimit WHERE COMPONENTID=999 AND YEAR=@YEAR)
		INSERT INTO @TAB1
			SELECT  @RGT+7,0,0,1000,'Standard Deduction',0,'TextBox','',0,(SELECT ISNULL(AmountLimit,0) FROM PAY_YearwiseTaxAmountLimit WHERE COMPONENTID=1000 AND YEAR=@YEAR)
