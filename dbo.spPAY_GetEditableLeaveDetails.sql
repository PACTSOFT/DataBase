...
BEGIN
	...
	SET NOCOUNT ON;
	DECLARE @EmployeeID INT,@LeaveType INT,@RCOUNT INT,@DocDate DATETIME,@FD DATETIME,@CCID BIGINT
	DECLARE @ALStartMonthYear DATETIME,@ALEndMonthYear DATETIME,@I INT,@TC INT
	DECLARE @TABLEAVES TABLE(ID BIGINT IDENTITY(1,1),CostCenterID BIGINT,DOCID BIGINT,INVDOCDETAILSID BIGINT,EMPID INT,LEAVETYPE INT,
	...
	END
		    ...
		    BEGIN
				...
				SELECT @RCOUNT=COUNT(*)	FROM   INV_DOCDETAILS ID WITH(NOLOCK), COM_DocCCData DC WITH(NOLOCK),COM_DocNumData DN WITH(NOLOCK),COM_DocTextData TD WITH(NOLOCK)
										WHERE  ID.INVDOCDETAILSID=DC.INVDOCDETAILSID AND ID.INVDOCDETAILSID=DN.INVDOCDETAILSID AND DC.INVDOCDETAILSID=DN.INVDOCDETAILSID AND
										       ID.INVDOCDETAILSID=TD.INVDOCDETAILSID  AND ID.STATUSID NOT IN (372,376)	AND ID.COSTCENTERID=@CCID
  				       						   AND CONVERT(DATETIME,@FD) between CONVERT(DATETIME,@ALStartMonthYear) and CONVERT(DATETIME,@ALEndMonthYear) 
