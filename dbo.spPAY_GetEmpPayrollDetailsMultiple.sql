...
DECLARE @DontConsiderDocs NVARCHAR(10),@DontConsiderDocsDays NVARCHAR(10)
...
BEGIN
	...
	IF EXISTS( SELECT  HistoryNodeID as PTDimension FROM COM_HistoryDetails WITH(NOLOCK) 
		...
		BEGIN
			...
			print @WhereCond
				SET @WhereCond=@WhereCond+' 
											AND c.dcCCNID'+CONVERT(NVARCHAR,(@HCCID-50000)) +' IN (SELECT 1 as CCNID UNION ALL Select CCNID'+CONVERT(NVARCHAR,(@HCCID-50000))+' FROM COM_CCCCData WITH(NOLOCK) WHERE CostCenterID=50051 AND NodeID IN(1,'+@EmpIDs+') ) '
			END
			ELSE
			BEGIN
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SET @WhereCond=@WhereCond+' 
											AND c.dcCCNID'+CONVERT(NVARCHAR,(@HCCID-50000)) +' IN (SELECT 1 as CCNID UNION ALL Select CCNID'+CONVERT(NVARCHAR,(@HCCID-50000))+' FROM COM_CCCCData WITH(NOLOCK) WHERE CostCenterID=50051 AND NodeID IN('+@EmpIDs+') ) '
			END
			ELSE
			BEGIN
				SET @WhereCond=@WhereCond+' 
...
Left Join COM_DocTextData b WITH(NOLOCK) on b.InvDocdetailsID=a.InvDocDetailsId
Left Join COM_DocCCData c WITH(NOLOCK) on c.InvDocdetailsID=a.InvDocDetailsId
Where a.StatusID=369 AND a.CostCenterID=40051  and isdate(ISNULL(b.dcAlpha1,''''))=1
...
WHERE a.StatusID=369 AND A.CostCenterID =40081 AND b.dcCCNID51 IN('+@EmpIDs+') AND ISDATE(ISNULL(TD.dcAlpha3,''''))=1 
...
INSERT INTO #TABAL
	SELECT EMPSEQNO,LeaveType,Total,0,0 FROM #TABTOPUP WHERE CONVERT(DATETIME,CONVERT(NVARCHAR,@PayrollMonth)) BETWEEN CONVERT(DATETIME,FromDate) AND CONVERT(DATETIME,ToDate)
...
JOIN COM_CC50052 D WITH(NOLOCK) ON D.NodeID=B.dcCCNID52
WHERE a.StatusID=369 AND A.DocumentType=62 AND ISDATE(ISNULL(c.dcAlpha4,''''))=1 AND ISDATE(ISNULL(c.dcAlpha5,''''))=1  AND B.dcCCNID51 IN('+@EmpIDs+') '
...
BEGIN
    ...
    WHERE a.DocumentType=73 and a.StatusID IN(369,371,441) AND ISNULL(D.DCALPHA4,'''')=''Yes'') '
END
...
LEFT JOIN ( SELECT b.dcCCNID51 as EmpSeqNo,b.dcCCNID52 as LoanType,CONVERT(DATETIME,d.dcAlpha4) as PayrollMonth,a.RefNo,c.dcNum1,SUM(c.dcNum3) as dcNum3
			...
			LEFT JOIN COM_DocTextData d WITH(NOLOCK) ON d.InvDocDetailsID=a.InvDocDetailsID
			LEFT JOIN COM_CC50052 e WITH(NOLOCK) ON e.NodeID=b.dcCCNID52
			WHERE a.StatusID=369 AND a.CostCenterID=40057 and b.dcCCNID51 IN('+@EmpIDs+') and isdate(ISNULL(d.dcAlpha4,''''))=1 and ISDATE(ISNULL(d.dcAlpha5,''''))=1 
...
WHERE a.CostCenterID=40056 AND b.dcCCNID51 IN('+@EmpIDs+')
...
BEGIN
	...
	IF EXISTS(SELECT  NodeID as EmpSeqNo,HistoryNodeID as PTDimension FROM COM_HistoryDetails WITH(NOLOCK) 
				WHERE NodeID IN('+@EmpIDs+')AND CostCenterID=50051 AND HistoryCCID='+@Val+'   
				AND (CONVERT(DATETIME,FromDate)<=CONVERT(DATETIME,'''+CONVERT(NVARCHAR,@PayrollMonth)+''') OR CONVERT(DATETIME,FromDate) BETWEEN CONVERT(DATETIME,'''+CONVERT(NVARCHAR,@PayrollStart)+''') AND CONVERT(DATETIME,'''+CONVERT(NVARCHAR,@PayrollEnd)+''')) 
				AND (CONVERT(DATETIME,ToDate)>=CONVERT(DATETIME,'''+CONVERT(NVARCHAR,@PayrollMonth)+''') OR ToDate IS NULL) )
	BEGIN
...
DN.dcNum11 as OT4,DN.dcNum12 as OT4Rate,DN.dcNum13 as OT5,DN.dcNum14 OT5Rate,DN.DCNUM15 TotalCost,dc.*
...
WHERE ID.STATUSID=369 AND LEN(TD.DCALPHA1)=11 AND ISDATE(ISNULL(TD.DCALPHA1,''''))=1 AND ISDATE(ISNULL(TD.DCALPHA2,''''))=1 AND ISDATE(ISNULL(TD.DCALPHA3,''''))=1 AND ID.DocumentType=67 
--ID.COSTCENTERID=40067
...
WHERE ID.INVDOCDETAILSID=TD.INVDOCDETAILSID AND ID.INVDOCDETAILSID=DC.INVDOCDETAILSID AND ID.INVDOCDETAILSID=DN.INVDOCDETAILSID AND TD.INVDOCDETAILSID=DC.INVDOCDETAILSID AND TD.INVDOCDETAILSID=DN.INVDOCDETAILSID AND ID.STATUSID=369
AND LEN(TD.DCALPHA1)=11 AND ISDATE(ISNULL(TD.DCALPHA1,''''))=1 AND ID.DocumentType=67 
--ID.COSTCENTERID=40067 
...
BEGIN	
	...
	SELECT  @MINHRSHDAY=ISNULL(TD.dcAlpha3,0),@MINHRSFDAY=ISNULL(TD.dcAlpha4,0) FROM INV_DOCDETAILS ID WITH(NOLOCK),COM_DocCCData DC WITH(NOLOCK),COM_DocTextData TD WITH(NOLOCK) WHERE ID.INVDOCDETAILSID=DC.INVDOCDETAILSID AND ID.INVDOCDETAILSID=TD.INVDOCDETAILSID AND DC.INVDOCDETAILSID=TD.INVDOCDETAILSID 
			AND ID.COSTCENTERID=40066 AND ISNULL(DC.DCCCNID51,1)=@EMPNODE AND ID.STATUSID=369 AND ISDATE(ISNULL(TD.dcAlpha6,''''))=1  
   		    AND CONVERT(DATETIME,TD.DCALPHA6) <=CONVERT(DATETIME,@FROMDATE) ORDER BY CONVERT(DATETIME,TD.dcAlpha6) 
   		    
   		    IF ISNULL(@MINHRSHDAY,0)<=0
			BEGIN
				SELECT  @MINHRSHDAY=ISNULL(TD.dcAlpha3,0),@MINHRSFDAY=ISNULL(TD.dcAlpha4,0) FROM INV_DOCDETAILS ID WITH(NOLOCK),COM_DocCCData DC WITH(NOLOCK),COM_DocTextData TD WITH(NOLOCK) WHERE ID.INVDOCDETAILSID=DC.INVDOCDETAILSID AND ID.INVDOCDETAILSID=TD.INVDOCDETAILSID AND DC.INVDOCDETAILSID=TD.INVDOCDETAILSID 
				AND ID.COSTCENTERID=40066 AND ISNULL(DC.DCCCNID51,1)=1 AND ID.STATUSID=369 AND ISDATE(ISNULL(TD.dcAlpha6,''''))=1  
	   		    AND CONVERT(DATETIME,TD.DCALPHA6) <=CONVERT(DATETIME,@FROMDATE) ORDER BY CONVERT(DATETIME,TD.dcAlpha6) 
			END
...
JOIN COM_DocNumData c WITH(NOLOCK) ON c.INVDOCDETAILSID=a.INVDOCDETAILSID
JOIN COM_DocTextData d WITH(NOLOCK) ON d.INVDOCDETAILSID=a.INVDOCDETAILSID
WHERE a.DocumentType=68 AND b.dcCCNID51 IN('+@EmpIDs+') AND a.StatusID=369 
...
JOIN COM_DocCCData b WITH(NOLOCK) ON b.INVDOCDETAILSID=a.INVDOCDETAILSID
JOIN COM_DocTextData d WITH(NOLOCK) ON d.INVDOCDETAILSID=a.INVDOCDETAILSID
WHERE ISNULL(d.dcAlpha17,'''')=''No'' AND b.dcCCNID51 IN('+@EmpIDs+') AND a.CostCenterID=40072 and a.StatusID=369 
...
SET @SQL=@SQL + '	GROUP BY a.DocID,b.dcCCNID51
	----
		...
		JOIN COM_DocCCData b WITH(NOLOCK) ON b.INVDOCDETAILSID=a.INVDOCDETAILSID
		JOIN COM_DocTextData d WITH(NOLOCK) ON d.INVDOCDETAILSID=a.INVDOCDETAILSID
		WHERE ISDATE(ISNULL(d.dcAlpha2,''''))=1 AND ISDATE(ISNULL(d.dcAlpha3,''''))=1 AND a.DocID IN(SELECT DocID From #TVAC2)
	----
	DROP TABLE #TVAC2
END
ELSE
...
JOIN COM_DocNumData c WITH(NOLOCK) ON c.INVDOCDETAILSID=a.INVDOCDETAILSID
JOIN COM_DocTextData d WITH(NOLOCK) ON d.INVDOCDETAILSID=a.INVDOCDETAILSID
WHERE ISDATE(ISNULL(d.dcAlpha10,''''))=1 AND ISNUMERIC(d.dcAlpha3)=1 AND ISNULL(d.dcAlpha9,'''')=''Yes'' 
AND a.CostCenterID=40058 AND b.dcCCNID51 IN('+@EmpIDs+') AND a.StatusID=369
...
JOIN COM_DocNumData c WITH(NOLOCK) ON c.INVDOCDETAILSID=a.INVDOCDETAILSID
JOIN COM_DocTextData d WITH(NOLOCK) ON d.INVDOCDETAILSID=a.INVDOCDETAILSID
WHERE ISDATE(ISNULL(d.dcAlpha1,''''))=1 
AND a.CostCenterID=40079 AND b.dcCCNID51 IN('+@EmpIDs+') AND a.StatusID=369
...
JOIN COM_DocNumData c WITH(NOLOCK) ON c.InvDocDetailsID=a.InvDocDetailsID
JOIN COM_DocTextData d WITH(NOLOCK) ON d.InvDocDetailsID=a.InvDocDetailsID
WHERE a.CostCenterID=40066 and a.StatusID=369 AND ISDATE(dcAlpha6)=1
...
JOIN COM_DocCCData b WITH(NOLOCK) ON b.INVDOCDETAILSID=a.INVDOCDETAILSID
JOIN COM_DocTextData d WITH(NOLOCK) ON d.INVDOCDETAILSID=a.INVDOCDETAILSID
WHERE  b.dcCCNID51 IN('+@EmpIDs+') AND a.CostCenterID=40072 AND a.StatusID=369 AND d.dcAlpha24=''Yes'' 
...
WHERE CostCenterID=40072 AND StatusID=369 AND LEN(T.DCALPHA2)<=15  AND ISDATE(ISNULL(T.DCALPHA2,''''))=1 AND ISNULL(T.dcAlpha1,'''')='''' AND C.dcCCNID51 IN('+@EmpIDs+')
...
JOIN COM_DOCCCDATA C ON C.INVDOCDETAILSID=I.INVDOCDETAILSID
JOIN COM_DOCTEXTDATA T ON T.INVDOCDETAILSID=I.INVDOCDETAILSID
WHERE CostCenterID=40072 AND StatusID=369 AND ISDATE(ISNULL(T.DCALPHA2,''''))=1 AND ISDATE(ISNULL(T.DCALPHA3,''''))=1 AND ISDATE(ISNULL(T.DCALPHA1,''''))=1 AND C.dcCCNID51 IN('+@EmpIDs+')
...
BEGIN
	...
	WHERE I.DOCUMENTTYPE=62 AND I.STATUSID IN(371,441) AND CC.dcCCNID51 IN('+@EmpIDs+')
	...
	JOIN COM_DocCCData CC WITH(NOLOCK) ON CC.InvDocDetailsID=I.InvDocDetailsID
	JOIN COM_DocTextData T WITH(NOLOCK) ON T.InvDocDetailsID=I.InvDocDetailsID
	WHERE I.CostCenterID=40072 AND I.STATUSID IN(371,441) AND CC.dcCCNID51 IN('+@EmpIDs+')
	...
	JOIN COM_DocCCData CC WITH(NOLOCK) ON CC.InvDocDetailsID=I.InvDocDetailsID
	JOIN COM_DocTextData T WITH(NOLOCK) ON T.InvDocDetailsID=I.InvDocDetailsID
	WHERE I.DOCUMENTTYPE=67 AND I.STATUSID IN(371,441) AND CC.dcCCNID51 IN('+@EmpIDs+')
	...
	JOIN COM_DocCCData CC WITH(NOLOCK) ON CC.InvDocDetailsID=I.InvDocDetailsID
	JOIN COM_DocTextData T WITH(NOLOCK) ON T.InvDocDetailsID=I.InvDocDetailsID
	WHERE I.DocumentType=68 AND I.STATUSID IN(371,441) AND CC.dcCCNID51 IN('+@EmpIDs+')
	...
	JOIN COM_DocCCData CC WITH(NOLOCK) ON CC.InvDocDetailsID=I.InvDocDetailsID
	JOIN COM_DocTextData T WITH(NOLOCK) ON T.InvDocDetailsID=I.InvDocDetailsID
	WHERE I.CostCenterID=40056 AND I.STATUSID IN(371,441) AND CC.dcCCNID51 IN('+@EmpIDs+')
	...
	JOIN COM_DocCCData CC WITH(NOLOCK) ON CC.InvDocDetailsID=I.InvDocDetailsID
	JOIN COM_DocTextData T WITH(NOLOCK) ON T.InvDocDetailsID=I.InvDocDetailsID
	WHERE I.CostCenterID=40057 AND I.STATUSID IN(371,441) AND CC.dcCCNID51 IN('+@EmpIDs+')
