...
GO
CREATE PROCEDURE [dbo].[spADM_GetFollowUPDashBoard]
...
set @sql='
	...
	INNER JOIN '+@table+'  ON '+@table+'.'+@Primarkey+'=
	F.CCNodeID AND F.CCID='+convert(nvarchar,@CCID)+'
	...
	exec (@sql)
	 ...
	 WHILE @I<=@COUNT
	 BEGIN
	 SELECT @ccnodeid=ccnodeid FROM #tbl WHERE id=@I
	 IF(NOT EXISTS(SELECT * FROM #TBLDUP WHERE CCNODEID=@ccnodeid))
	 BEGIN
	 ...
	 set @I=@I+1
	 END
	 select * from #tbl WHERE ISREPEAT=0 
	 drop table #tbl
	 DROP TABLE #TBLDUP
