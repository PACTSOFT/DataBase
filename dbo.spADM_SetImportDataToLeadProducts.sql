...
GO
CREATE PROCEDURE [dbo].[spADM_SetImportDataToLeadProducts]
...
SET NOCOUNT ON;
		--Declaration Section  
		DECLARE @Dt FLOAT ,@XML XML,@LeadID bigint,@Qty float,@UOMID BIGINT,@SQL NVARCHAR(MAX), @return_value INT
		DECLARE @NodeID bigint,@BINID INT,@ExtraFields NVARCHAR(MAX),@tempCode NVARCHAR(300),@PRODUCTID INT,@BIN NVARCHAR(300), @I INT,@COUNT INT,@CCID BIGINT,
		@PRODUCTNAME NVARCHAR(300),@CRMPRODUCTNAME NVARCHAR(300),@DESCR NVARCHAR(300),  @TableName nvarchar(300),@CostCenterId bigint
		SET @Dt=CONVERT(FLOAT,GETDATE())
		
		SELECT @CostCenterId=ISNULL(VALUE,'0') FROM ADM_GLOBALPREFERENCES WHERE NAME='CRM-Products'
		...
		SELECT Top 1 @TableName=SysTableName FROM ADM_CostCenterDef WITH(NOLOCK) WHERE CostCenterID=@CostCenterId  
		 ...
		 if @IsCode=0
			SELECT @LeadID=LEADID FROM CRM_LEADS WHERE Company=LTRIM(RTRIM(@AccountName))
	    else 
			SELECT @LeadID=LEADID FROM CRM_LEADS WHERE Code=LTRIM(RTRIM(@AccountCode))
		...
		SELECT
		 ...
		 END	 
	  	    ...
	  	    SET @SQL=' select @return_value=NodeID  from '+@TableName+' WHERE replace(NAME,'' '','''')=replace('''+@CRMPRODUCTNAME+''' ,'' '','''')'  
