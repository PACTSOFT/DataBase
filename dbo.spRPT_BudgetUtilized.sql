...
SET NOCOUNT ON;  
	...
	--BudgetTypeID
	SELECT @NumDimensions=NumDimensions,@IsQtyBudget=QtyBudget,@AccountTypes=AccountTypes,
	@FromDate=CONVERT(DATETIME,FinYearStartDate) FROM COM_BudgetDef WITH(NOLOCK)
	...
	SELECT  @CCWHERE=@CCWHERE+' AND DCC.dcCCNID'+CONVERT(NVARCHAR,CostCenterID-50000)+'=T.CCNID'+CONVERT(NVARCHAR,CostCenterID-50000) ,
			@TblCols=@TblCols+' ,CCNID'+CONVERT(NVARCHAR,CostCenterID-50000)+' BIGINT'
...
BEGIN
	...
	INNER JOIN COM_BudgetAlloc T WITH(NOLOCK) ON T.BudgetDefID=@BudgetID '+@CCWHERE+'
...
BEGIN
	...
	declare @CommonAccount bigint,@BudDim int,@sqlAccs nvarchar(max),@CAFrom1 nvarchar(max),@CAFrom2 nvarchar(max),@AccWhere1 nvarchar(max),@AccWhere2 nvarchar(max)
	...
	CREATE TABLE #tblMapAccs(ID BIGINT,BudDimID bigint)
	...
	begin
		...
		where B.BudgetDefID='+CONVERT(NVARCHAR,@BudgetID)+' and AccountID='+convert(nvarchar,@CommonAccount)+' and ParentCostCenterID=2 and CostCenterID='+convert(nvarchar,@BudDim)
	...
	INNER JOIN COM_BudgetAlloc T WITH(NOLOCK) ON T.BudgetDefID=@BudgetID '+@CCWHERE+@CAFrom1+'
	...
	INNER JOIN COM_DocCCData DCC WITH(NOLOCK) ON DCC.AccDocDetailsID=D.AccDocDetailsID 
	INNER JOIN COM_BudgetAlloc T WITH(NOLOCK) ON T.BudgetDefID=@BudgetID '+@CCWHERE+@CAFrom2+'
	...
	inner join COM_DocNumData N with(nolock) on N.InvDocDetailsID=D.InvDocDetailsID
	INNER JOIN COM_DocCCData DCC with(nolock) ON DCC.InvDocDetailsID=D.InvDocDetailsID
	INNER JOIN COM_BudgetAlloc T WITH(NOLOCK) ON T.BudgetDefID='+CONVERT(NVARCHAR,@BudgetID)+' '+@CCWhere+'
	...
	INNER JOIN COM_DocCCData DCC with(nolock) ON DCC.InvDocDetailsID=D.InvDocDetailsID
	INNER JOIN COM_BudgetAlloc T WITH(NOLOCK) ON T.BudgetDefID=@BudgetID '+@CCWhere+@CAFrom1+'
	where D.CostCenterID IN ('+@InvAccDocs+') and '+@DateField+'>=@From AND '+@DateField+'<=@To'+@AccWhere1+@StatusWhere+'
	UNION ALL
	...
	INNER JOIN COM_DocCCData DCC with(nolock) ON DCC.InvDocDetailsID=D.InvDocDetailsID
	INNER JOIN COM_BudgetAlloc T WITH(NOLOCK) ON T.BudgetDefID=@BudgetID '+@CCWhere+@CAFrom2+'
	where D.CostCenterID IN ('+@InvAccDocs+') and '+@DateField+'>=@From AND '+@DateField+'<=@To'+@AccWhere2+@StatusWhere+''
		end
	end
...
INNER JOIN COM_BudgetAlloc T WITH(NOLOCK) ON T.BudgetDefID=@BudgetID '+@CCWhere+replace(@CAFrom1,'D.','INV.')+'
...
inner join COM_DocNumData N with(nolock) on N.InvDocDetailsID=INV.InvDocDetailsID
INNER JOIN COM_DocCCData DCC with(nolock) ON DCC.InvDocDetailsID=INV.InvDocDetailsID
INNER JOIN COM_BudgetAlloc T WITH(NOLOCK) ON T.BudgetDefID=@BudgetID '+@CCWhere+replace(@CAFrom2,'D.','INV.')+'
