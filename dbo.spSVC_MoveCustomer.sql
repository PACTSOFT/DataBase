...
GO
CREATE PROCEDURE [dbo].[spSVC_MoveCustomer]
	@CustomerID [bigint],
...
SET NOCOUNT ON; 
		...
		INSERT INTO @Temp  
		SELECT CustomerID FROM [SVC_Customers] WITH(NOLOCK) WHERE lft >= @lft AND rgt <= @rgt  
		...
		BEGIN  
				...
				BEGIN  
					...
					UPDATE SVC_Customers  SET rgt=rgt+@Width+1 WHERE rgt>@Selectedlft and rgt<@lft  
					UPDATE SVC_Customers  SET lft=lft+@Width+1 WHERE lft>@Selectedlft and lft<@lft  

					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE SVC_Customers  SET  Depth=Depth -@Depth+@SelectedDepth +1, rgt=rgt- @lft+@Selectedlft+1,lft=lft- @lft+@Selectedlft+1  
					WHERE CustomerID in (SELECT ID FROM @Temp)  

					--SET PARENT AS SELECTED NODE IF IT IS GROUP
					UPDATE SVC_Customers  SET ParentID=@SelectedNodeID  
				...
				BEGIN   
					--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND PRODUCT
					UPDATE SVC_Customers  SET rgt=rgt+@Width+1 WHERE rgt>@Selectedlft+1 and rgt<@lft  
					UPDATE SVC_Customers  SET lft=lft+@Width+1 WHERE lft>@Selectedlft and lft<@lft  

					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE SVC_Customers  SET Depth=Depth -@Depth+@SelectedDepth,rgt=rgt- @lft+@Selectedrgt+1,lft=lft- @lft+@Selectedrgt+1  
					WHERE CustomerID in (SELECT ID FROM @Temp)  

					--SET PARENTID OF SELECTED NODE IF IT IS NOT GROUP
					UPDATE SVC_Customers  SET ParentID=@ParentID  
					WHERE CustomerID=@CustomerID  
				END  
		END  
		ELSE  --IF MOVE DOWN 
		BEGIN  
				...
				BEGIN  
					--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND PRODUCT
					UPDATE SVC_Customers  SET rgt=rgt-@Width-1 WHERE rgt>@rgt and rgt<=@Selectedlft  
					UPDATE SVC_Customers  SET lft=lft-@Width-1 WHERE lft>@rgt and lft<=@Selectedlft  

					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE SVC_Customers  SET  Depth=Depth -@Depth+@SelectedDepth +1, rgt=rgt+(@Selectedlft-@Width)-@lft,lft=lft+(@Selectedlft-@Width)-@lft  
					WHERE CustomerID in (SELECT ID FROM @Temp)  
				...
				BEGIN   
					--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND PRODUCT
					UPDATE SVC_Customers  SET rgt=rgt-@Width-1 WHERE rgt>@rgt and rgt<=@Selectedlft+1  
					UPDATE SVC_Customers  SET lft=lft-@Width-1 WHERE lft>@rgt and lft<=@Selectedlft  

					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE SVC_Customers  SET  Depth=Depth -@Depth+@SelectedDepth , rgt=rgt+(@Selectedlft-@Width)-@lft+1,lft=lft+(@Selectedlft-@Width)-@lft+1  
					WHERE CustomerID in (SELECT ID from @Temp)  
