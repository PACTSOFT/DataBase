...
BEGIN TRY
	...
	DECLARE @Grade INT,@Payrolltype VARCHAR(50),@AssignedLeaves FLOAT,@AvlblLeaves FLOAT,@PermonthLeaves FLOAT,@ExtraAssignedLeaves INT
	DECLARE @PermonthLeavesRem FLOAT,@MonthNo INT,@MonthSNo INT,@CurrYearLeavestaken FLOAT,@MonthLeavesrem FLOAT,@CarryforwardLeaves FLOAT,@MaxCarryForwardDays FLOAT
...
END
	...
	BEGIN
		...
		BEGIN
			SELECT @TotEncashDays=ISNULL(TD.dcAlpha3,0) FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DocCCData DC WITH(NOLOCK) ON ID.INVDOCDETAILSID=DC.INVDOCDETAILSID
				   JOIN COM_DocNumData DN WITH(NOLOCK) ON ID.INVDOCDETAILSID=DN.INVDOCDETAILSID JOIN COM_DocTextData TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID
			WHERE  ID.costcenterid=40058 AND DC.DCCCNID51=@EmployeeID AND DC.DCCCNID52=@LeaveType AND convert(datetime,id.docdate) between convert(datetime,@PayrollDate) and convert(datetime,@EDATE)
		END
		...
		BEGIN
			...
			SELECT @DOCIDNOOFDAYS=ISNULL(TD.dcAlpha3,0),@ActualAvailableLeaves=isnull(TD.dcAlpha7,0) FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DocCCData DC WITH(NOLOCK) ON ID.INVDOCDETAILSID=DC.INVDOCDETAILSID
				   JOIN COM_DocNumData DN WITH(NOLOCK) ON ID.INVDOCDETAILSID=DN.INVDOCDETAILSID JOIN COM_DocTextData TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID
			WHERE  ID.costcenterid=40058 AND DC.DCCCNID51=@EmployeeID AND DC.DCCCNID52=@LeaveType  AND ID.DOCID=@DocID
		END
	END
	...
	--START : LOADING ASSIGNED LEAVES       
		INSERT INTO @TABASSIGNEDLEAVES	       	       
			...
			WHERE  ISDATE(TD.DCALPHA3)=1 AND ISDATE(TD.DCALPHA4)=1 AND CONVERT(DATETIME,@Date) between CONVERT(DATETIME,TD.DCALPHA3) and CONVERT(DATETIME,TD.DCALPHA4)
  		           AND ID.COSTCENTERID=40060 AND ID.STATUSID=369 AND DC.DCCCNID51=@EmployeeID AND DC.DCCCNID52=@LeaveType --AND DC.DCCCNID53=@Grade
		...
		INSERT INTO @TABASSIGNEDLEAVES     	       
			...
			FROM   INV_DOCDETAILS ID WITH(NOLOCK) INNER JOIN COM_DocCCData DC WITH(NOLOCK) ON ID.INVDOCDETAILSID=DC.INVDOCDETAILSID
				   INNER JOIN COM_DocNumData DN WITH(NOLOCK) ON ID.INVDOCDETAILSID=DN.INVDOCDETAILSID INNER JOIN COM_DocTextData TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID
			WHERE  ISDATE(TD.DCALPHA3)=1 AND ISDATE(TD.DCALPHA4)=1 AND CONVERT(DATETIME,@Date) between CONVERT(DATETIME,TD.DCALPHA3) and CONVERT(DATETIME,TD.DCALPHA4)
  		           AND ID.COSTCENTERID=40081 AND ID.STATUSID=369 AND DC.DCCCNID51=@EmployeeID AND DC.DCCCNID52=@LeaveType --AND DC.DCCCNID53=@Grade	  		           
		...
		SELECT @FROMDATETABLE=CONVERT(DATETIME,EXTFROMDATE) from @TABASSIGNEDLEAVES where  NOOFMONTHS=12

		--IF EMPLOYEE JOINED BETWEEN FISCAL YEAR	
		IF(@AssignedLeaves=0)
			SELECT @AssignedLeaves=isnull(SUM(AssignedLeaves),0),@FROMDATETABLE=CONVERT(DATETIME,@EMPDOJ) from @TABASSIGNEDLEAVES where  CONVERT(DATETIME,@EMPDOJ) BETWEEN CONVERT(DATETIME,DATEADD(MONTH,DATEDIFF(MONTH,0,CONVERT(DATETIME,EXTFROMDATE)),0)) AND CONVERT(DATETIME,DATEADD(D,-1,DATEADD(MM,DATEDIFF(M,0,EXTFROMDATE)+1,0)))
	...
	BEGIN			
		...
		BEGIN		
				...
				print @DOCIDNOOFDAYS
				   ...
				   BEGIN
						...
						WHERE  ISDATE(TD.DCALPHA3)=1 AND ISDATE(TD.DCALPHA4)=1 AND ID.STATUSID=369 AND CONVERT(DATETIME,TD.DCALPHA3) BETWEEN CONVERT(DATETIME,@FDYear) AND CONVERT(DATETIME,@TDYear)
							   AND CC.DCCCNID51=@EmployeeID AND CC.DCCCNID52=@LeaveType AND ID.COSTCENTERID=40060 AND ID.CommonNarration='#Opening#'
						...
						FROM   INV_DOCDETAILS ID WITH(NOLOCK) INNER JOIN COM_DocCCData CC WITH(NOLOCK) ON CC.INVDOCDETAILSID=ID.INVDOCDETAILSID
							   INNER JOIN COM_DocNumData DN WITH(NOLOCK) ON DN.INVDOCDETAILSID=ID.INVDOCDETAILSID INNER JOIN COM_DocTextDATA TD WITH(NOLOCK) ON TD.INVDOCDETAILSID=ID.INVDOCDETAILSID
						WHERE  ISDATE(TD.DCALPHA3)=1 AND ISDATE(TD.DCALPHA4)=1 AND ID.STATUSID=369 AND CONVERT(DATETIME,TD.DCALPHA3) BETWEEN CONVERT(DATETIME,@FDYear) AND CONVERT(DATETIME,@TDYear)
							   AND CC.DCCCNID51=@EmployeeID AND CC.DCCCNID52=@LeaveType AND ID.COSTCENTERID=40060 AND ID.CommonNarration='#CarryForward#'
						...
						FROM   INV_DOCDETAILS ID WITH(NOLOCK) INNER JOIN COM_DocCCData CC WITH(NOLOCK) ON CC.INVDOCDETAILSID=ID.INVDOCDETAILSID
							   INNER JOIN COM_DocNumData DN WITH(NOLOCK) ON DN.INVDOCDETAILSID=ID.INVDOCDETAILSID INNER JOIN COM_DocTextDATA TD WITH(NOLOCK) ON TD.INVDOCDETAILSID=ID.INVDOCDETAILSID
						WHERE  ISDATE(TD.DCALPHA3)=1 AND ISDATE(TD.DCALPHA4)=1 AND ID.STATUSID=369 AND CONVERT(DATETIME,TD.DCALPHA3) BETWEEN CONVERT(DATETIME,@FDYear) AND CONVERT(DATETIME,@TDYear)
							   AND CC.DCCCNID51=@EmployeeID AND CC.DCCCNID52=@LeaveType AND ID.COSTCENTERID=40081
		...
		SELECT ISNULL(AssignedLeaves,0) AssignedLeaves,ISNULL(AvailableLeaves,0) AvailableLeaves,CONVERT(DATETIME,getdate()) as FromDate,CONVERT(DATETIME,getdate()) as ToDate,
		       @MaxEncashLeaves as MaxEncashLeaves,@MaxLeaves as MaxLeaves,ISNULL(@ActualAvailableLeaves,0) AS ActualAvailableLeaves,@LEThresholdLimit as ThresholdLimit FROM @GETLEAVES
    END
