...
SET NOCOUNT ON;         
 ...
 BEGIN    
   ...
   ,[Subject],Body,ExtendedQuery,FieldsXML,CostCenterID,ReportID,SendReportAs,IgnoreBasedOn,UserWiseDims,UserWiseDimsCond,UIXML,IsApproveButton  
   ,CompanyGUID    
   ,[GUID]    
   ,CreatedBy    
   ,CreatedDate)    
  VALUES    
   (@TemplateName,@TemplateType,@StatusID    
   ,@From,@DisplayName,@To,@CC,@BCC,@IgnoreMailsTo,@AttachmentType,@AttachmentID    
   ,@Subject,@Body,@Query,@FieldsXML,@CostCenterID,@ReportID,@SendReportAs,@IgnoreBasedOn,@Message,@UserWiseDimsCond,@UIXML,@IsApproveButton  
   ,@CompanyGUID    
   ,newid()    
   ,@UserName    
   ,@Dt)          
