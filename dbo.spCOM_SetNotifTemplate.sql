...
SET NOCOUNT ON;       
 --Declaration Section      
 DECLARE @HasAccess BIT,@Dt FLOAT,@XML XML  
 ...
 BEGIN  
   ...
   ,Subject,Body,ExtendedQuery,FieldsXML,CostCenterID,ReportID,SendReportAs,IgnoreBasedOn,UserWiseDims,UIXML,IsApproveButton
   ,CompanyGUID  
   ,GUID  
   ,CreatedBy  
   ,CreatedDate)  
  VALUES  
   (@TemplateName,@TemplateType,@StatusID  
   ,@From,@DisplayName,@To,@CC,@BCC,@IgnoreMailsTo,@AttachmentType,@AttachmentID  
   ,@Subject,@Body,@Query,@FieldsXML,@CostCenterID,@ReportID,@SendReportAs,@IgnoreBasedOn,@Message,@UIXML,@IsApproveButton
   ,@CompanyGUID  
   ,newid()  
   ,@UserName  
   ,@Dt)        
 ...
 BEGIN  
  ...
  SET TemplateName=@TemplateName,StatusID=@StatusID,[From]=@From,DisplayName=@DisplayName, [To]=@To, CC=@CC, BCC=@BCC, IgnoreMailsTo=@IgnoreMailsTo, AttachmentType=@AttachmentType,AttachmentID=@AttachmentID,  
   Subject=@Subject,Body=@Body,ExtendedQuery=@Query,FieldsXML=@FieldsXML,CostCenterID=@CostCenterID,ReportID=@ReportID,SendReportAs=@SendReportAs,IgnoreBasedOn=@IgnoreBasedOn,
   UserWiseDims=@Message,UIXML=@UIXML,IsApproveButton=@IsApproveButton,GUID=newid(),ModifiedBy=@UserName,ModifiedDate=@Dt
 ...
 BEGIN  
  ...
  select @TemplateID,X.value('@ActionID','bigint')  
