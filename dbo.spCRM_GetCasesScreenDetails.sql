...
SET NOCOUNT ON
 ...
 create table #tblUsers(username nvarchar(100))
		...
		select * from CRM_Cases WITH(NOLOCK) where IsGroup=1 
		SELECT Name,Value from  COM_CostCenterPreferences WITH(NOLOCK) WHERE CostCenterID=73 
			...
			while @i<=@cnt
			begin
				select @ID=ServiceTypeID, @Tech=Technicians from #temp where id=@i
				insert into #tempTech  
				EXEC SPSplitString @Tech,',' 
				insert into #Fin 
				select @ID,TechID from #tempTech
				truncate table #tempTech 
			set @i=@i+1
			end
			drop table #temp 
			drop table #tempTech
			--select s.ServiceTypeID, ServiceName, Technician from #Fin 
			--left join CRM_ServiceTypes s on s.ServiceTypeID=#Fin.ServiceTypeID
		 ...
		 BEGIN
			...
			(select value from COM_CostCenterPreferences with(nolock) where CostCenterID=82 and name='ServiceTypeLinkDimension')
			 SET @SQL=' SELECT NODEID,NAME Technician, f.ServiceTypeID FROM '+@TableNm +' t 
			   join #Fin f on f.Technician=t.NodeID  '
			  print (@SQL)
			 EXEC(@SQL)
			 drop table #Fin
		 END
		 ...
		 SELECT NULL
		 SELECT NodeID, Name,IsDefault FROM COM_Lookup WITH(NOLOCK) WHERE LookupType=22
		 SELECT NodeID, Name,IsDefault FROM COM_Lookup WITH(NOLOCK) WHERE LookupType=23
		 SELECT NodeID, Name,IsDefault FROM COM_Lookup WITH(NOLOCK) WHERE LookupType=24
		...
		BEGIN
			...
			convert(datetime,AssignedDate) as ADate,convert(datetime,c.CreateDate) as CDAte,
			 dbo.fnGet_GetAssignedListForFeatures(73,c.CaseID) as AssignedUser, 
			A.AccountName as Customer
			FROM CRM_Cases c WITH(NOLOCK)
			left join Acc_accounts A on c.customerid=A.Accountid
			LEFT JOIN ADM_USERS U ON U.USERID=C.ASSIGNEDTO
			LEFT JOIN INV_Product p ON c.ProductID=p.ProductID
			LEFT JOIN INV_DocDetails d ON c.SvcContractID=d.DocID
			where CaseID=@CaseID
			
			set @SQL='		SELECT *,R.Reason, T.Name NODEID_Key FROM dbo.CRM_CaseSvcTypeMap WITH(NOLOCK) 
			...
			print (@SQL)
			exec (@SQL)
			SELECT * FROM dbo.CRM_CasesExtended WITH(NOLOCK) where CaseID=@CaseID
		...
		WHERE NodeID=@CaseID and CostCenterID=73
      ...
      L.CCNID42 as CCNID42_Key,L.CCNID43 as CCNID43_Key ,L.CCNID44 as CCNID44_Key ,L.CCNID45 as CCNID45_Key ,L.CCNID46 as CCNID46_Key ,L.CCNID47 as CCNID47_Key ,L.CCNID48 as CCNID48_Key ,L.CCNID49 as CCNID49_Key ,L.CCNID50 as CCNID50_Key ,
	   NID1.NAME as CCNID1,NID2.NAME as CCNID2,NID3.NAME as CCNID3,NID4.NAME as CCNID4,NID5.NAME as CCNID5,NID6.NAME as CCNID6,NID7.NAME as CCNID7,NID8.NAME as CCNID8,NID9.NAME as CCNID9,NID10.NAME as CCNID10,
		...
		LEFT JOIN COM_CC50049 NID49 ON L.CCNID49=NID49.NODEID 
		LEFT JOIN COM_CC50050 NID50 ON L.CCNID50=NID50.NODEID 
		WHERE L.CCNodeID=@CaseID and CCID=73
	 ...
	 SELECT NodeID, Name,IsDefault FROM COM_Lookup WITH(NOLOCK) WHERE LookupType=42
SET NOCOUNT OFF;
RETURN 1
END TRY
BEGIN CATCH  
