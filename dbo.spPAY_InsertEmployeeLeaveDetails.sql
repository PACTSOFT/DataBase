...
CREATE TABLE #TABEMP(ID BIGINT IDENTITY(1,1),EMPNODE INT)
...
BEGIN
	...
	BEGIN
		...
		CREATE TABLE #TABAL(ID BIGINT IDENTITY(1,1),EMPNODE INT,LEAVETYPE INT,LEAVETYPENAME VARCHAR(100),LEAVES FLOAT)
		CREATE TABLE #TABTOPUP (ID BIGINT IDENTITY(1,1),EMPNODE INT,LEAVETYPE INT,LEAVETYPENAME VARCHAR(100),LEAVES FLOAT)
		CREATE TABLE #TABPAL(EMPSEQNO INT,LeaveType VARCHAR(100),OPBALANCE FLOAT,Deducted FLOAT,Balance FLOAT)
		CREATE TABLE #TEMPEMPLOYEELEAVEDETAILS(EmployeeID bigint,LeaveTypeID bigint,LeaveYear datetime,DeductedLeaves float,BalanceLeaves float)
		...
		WHERE CC.DCCCNID51=@EMPLOYEEID AND ID.STATUSID=369 AND ID.COSTCENTERID=40060 AND ISDATE(TD.dcAlpha3)=1 AND CONVERT(DATETIME,TD.dcAlpha3) between CONVERT(DATETIME,@ALStartMonthYear) and CONVERT(DATETIME,@ALEndMonthYear)
		...
		FROM   INV_DOCDETAILS ID WITH(NOLOCK) INNER JOIN COM_DocTextData TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID
		       ...
		       INNER JOIN COM_DOCNUMDATA DN WITH(NOLOCK) ON ID.INVDOCDETAILSID=DN.INVDOCDETAILSID
		       INNER JOIN COM_CC50052 C52 WITH(NOLOCK) ON C52.NODEID=CC.DCCCNID52
		WHERE CC.DCCCNID51=@EMPLOYEEID AND CC.DCCCNID51<>1  AND ID.STATUSID=369 AND ID.COSTCENTERID=40081 AND ISDATE(TD.dcAlpha3)=1 AND CONVERT(DATETIME,TD.dcAlpha3) between CONVERT(DATETIME,@ALStartMonthYear) and CONVERT(DATETIME,@ALEndMonthYear)
		GROUP BY CC.DCCCNID51,CC.DCCCNID52,C52.NAME
