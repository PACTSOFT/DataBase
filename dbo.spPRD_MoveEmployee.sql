...
GO
CREATE PROCEDURE [dbo].[spPRD_MoveEmployee]
	@EmployeeID [bigint],
...
SET NOCOUNT ON; 
		...
		IF(@EmployeeID=0 OR @SelectedNodeID=0)
		BEGIN  
			RAISERROR('-100',16,1)   
		END
		...
		BEGIN
			...
			FROM PRD_Resources WITH(NOLOCK) WHERE ResourceID=@EmployeeID  
		...
		--GETTING THE CHILDS OF PRODUCTS 
			...
			BEGIN  
					...
					BEGIN  
						...
						UPDATE PRD_Resources  SET rgt=rgt+@Width+1 WHERE rgt>@Selectedlft and rgt<@lft  
						UPDATE PRD_Resources  SET lft=lft+@Width+1 WHERE lft>@Selectedlft and lft<@lft  

						--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
						UPDATE PRD_Resources  SET  Depth=Depth -@Depth+@SelectedDepth +1, rgt=rgt- @lft+@Selectedlft+1,lft=lft- @lft+@Selectedlft+1  
						WHERE ResourceID in (SELECT ID FROM @Temp)  

						--SET PARENT AS SELECTED NODE IF IT IS GROUP
						UPDATE PRD_Resources  SET ParentID=@SelectedNodeID  
					...
					BEGIN   
						--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND PRODUCT
						UPDATE PRD_Resources  SET rgt=rgt+@Width+1 WHERE rgt>@Selectedlft+1 and rgt<@lft  
						UPDATE PRD_Resources  SET lft=lft+@Width+1 WHERE lft>@Selectedlft and lft<@lft  

						--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
						UPDATE PRD_Resources  SET Depth=Depth -@Depth+@SelectedDepth,rgt=rgt- @lft+@Selectedrgt+1,lft=lft- @lft+@Selectedrgt+1  
						WHERE ResourceID in (SELECT ID FROM @Temp)  
			...
			BEGIN  
					...
					BEGIN  
						--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND PRODUCT
						UPDATE PRD_Resources  SET rgt=rgt-@Width-1 WHERE rgt>@rgt and rgt<=@Selectedlft  
						UPDATE PRD_Resources  SET lft=lft-@Width-1 WHERE lft>@rgt and lft<=@Selectedlft  

						--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
						UPDATE PRD_Resources  SET  Depth=Depth -@Depth+@SelectedDepth +1, rgt=rgt+(@Selectedlft-@Width)-@lft,lft=lft+(@Selectedlft-@Width)-@lft  
						WHERE ResourceID in (SELECT ID FROM @Temp)  
					...
					BEGIN   
						--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND PRODUCT
						UPDATE PRD_Resources  SET rgt=rgt-@Width-1 WHERE rgt>@rgt and rgt<=@Selectedlft+1  
						UPDATE PRD_Resources  SET lft=lft-@Width-1 WHERE lft>@rgt and lft<=@Selectedlft  

						--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
						UPDATE PRD_Resources  SET  Depth=Depth -@Depth+@SelectedDepth , rgt=rgt+(@Selectedlft-@Width)-@lft+1,lft=lft+(@Selectedlft-@Width)-@lft+1  
						WHERE ResourceID in (SELECT ID from @Temp)  
