...
SET NOCOUNT ON 
    ...
    declare @RenewRefID BIGINT,@LinkedQuotationID BIGINT,@isContractRenewed BIT,@PROPERTYID BIGINT , @UnitID BIGINT,@IVAR BIT,@Uname nvarchar(max),@sivccid int
    Declare @WID bigint,@Userlevel int,@StatusID int,@Level int,@canApprove bit,@canEdit bit,@Type int,@escDays int,@CreatedDate datetime,@IsRecurPosted bit,@isopening int
    
	declare @stat int,@table nvarchar(50),@sql nvarchar(max),@dimCid int,@salsDimID int,@colnames nvarchar(max)
	set @stat=0
	set @dimCid=0
	set @sivccid=0
	set @salsDimID=0
	...
	SNO ,(SELECT MAX(SNO) + 1 FROM REN_Contract) AS NEWSNO,RenewRefID,IsExtended,RentAmt
	...
	,convert(datetime,ExpectedEndDate) ExpectedEndDate,GracePeriod,IsOpening,wfaction
	...
	,CP.SPType,TaxableAmt,CP.RecurInvoice,CP.PostDebit,CP.InclChkGen,CP.VatPer,CP.VatAmount, ACC.ACCOUNTNAME CREDITNAME , ACCD.ACCOUNTNAME  DEBITNAME ,CP.IsRecurr ,PARTP.Refund  ,PART.Refund UnitRefund , Sts.Status StatusID'
	...
	BEGIN
		...
		, Convert(datetime,CP.TillDate) TillDate,  ADF.DocumentName DocumentName,Doc.OpPdcStatus,Doc.DocID DocID , Doc.StatusID   DocStatusID,Doc.DocumentType  DocumentType,Doc.BillDate BillNo,Convert(datetime,Doc.DocDate) PostingDate,RentAmount,cp.LocationID,loc.name locname,
		DimNodeID,SalesManID'+@colnames+'
	...
	ELSE
	BEGIN
		set @sql='SELECT   DISTINCT CDM.SNO, CP.NodeID, CP.ContractID, CP.ChequeNo, Convert(datetime,CP.ChequeDate) ChequeDate , CP.CustomerBank, CP.DebitAccID, CP.Amount ,  CP.GUID ,                      
		Period,ACC.AccountName DebitAccName , CP.Narration , Sts.Status StatusID   , Doc.VoucherNo ,Doc.DocPrefix DocPrefix ,Doc.DocNumber DocNumber , doc.CostCenterID CostCenterID,cp.Particular
