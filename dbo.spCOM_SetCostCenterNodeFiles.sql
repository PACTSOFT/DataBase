...
SET NOCOUNT ON; 
		...
		BEGIN    
			...
			INSERT INTO COM_Files
					...
					,IssueDate,Type,RefNum,Remarks,DocNo)    
			VALUES
					(@FilePath
					,@ActualFileName
					,@RelativeFileName
					...
					,CONVERT(FLOAT,@IssueDate),@Type,@RefNum,@Remarks,@DocNo)  
			...
			BEGIN    
				...
				BEGIN
					...
					SET  ActualFileName=@ActualFileName 
						...
						,IssueDate=CONVERT(FLOAT,@IssueDate),Type=@Type,RefNum=@RefNum,Remarks=@Remarks,DocNo=@DocNo
						WHERE FileID=@FileID  
				END
				ELSE 
				BEGIN
