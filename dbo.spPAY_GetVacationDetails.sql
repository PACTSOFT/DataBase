...
BEGIN TRY    
SET NOCOUNT ON;
DECLARE @GRADE INT,@GRADEWISEVACATIONPREF VARCHAR(5),@PayrollDate DATETIME,@DOConfirmation DATETIME,@OPLeavesAsOn DATETIME
...
IF ((SELECT COUNT(*) FROM COM_CC50051 WITH(NOLOCK) where NodeID=@EmpNode and isnull(VacDaysPerMonth,0)>0)<=0)
...
IF ((SELECT COUNT(*) FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DOCTEXTDATA TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID JOIN COM_DOCCCDATA CC  WITH(NOLOCK) ON ID.INVDOCDETAILSID=CC.INVDOCDETAILSID WHERE  CC.DCCCNID53=@GRADE AND ID.COSTCENTERID=40061 AND ID.StatusID=369 )<=0)
...
IF ((SELECT count(*)  FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DOCTEXTDATA TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID JOIN COM_DOCCCDATA CC  WITH(NOLOCK) ON ID.INVDOCDETAILSID=CC.INVDOCDETAILSID WHERE  CC.DCCCNID51=@EmpNode AND ID.COSTCENTERID=40072 AND ID.StatusID=369
...
INSERT INTO @TABCP
	...
	WHERE  PC.NODEID=CP.COMPONENTID --AND  ISNUMERIC(TD.DCALPHA17)=1 
	AND CP.COMPONENTID=TD.DCALPHA17 AND DN.DCNUM1<>0 AND CP.GRADEID=@GRADE AND NAME <> 'EMPLOYEE_BASIC' AND ID.COSTCENTERID=40061 AND ID.StatusID=369 AND CD.dcCCNID53=@GRADE
...
--TABLE5
SELECT @OPLeavesAsOn=CONVERT(DateTime,OPLeavesAsOn) FROM COM_CC50051 WHERE NODEID=@EmpNode
...
IF ((SELECT count(*)  FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DOCTEXTDATA TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID JOIN COM_DOCCCDATA CC  WITH(NOLOCK) ON ID.INVDOCDETAILSID=CC.INVDOCDETAILSID
	 WHERE  CC.DCCCNID51=@EmpNode AND ID.COSTCENTERID=40072 AND ID.StatusID=369 AND isnull(TD.DCALPHA16,'')='No' AND ISNULL(TD.DCALPHA2,'')<>'' AND ISNULL(TD.DCALPHA3,'')<>'' AND ISNULL(TD.DCALPHA1,'')<>'' AND CONVERT(DATETIME,TD.DCALPHA3)<CONVERT(DATETIME,@FromDate))>0)
BEGIN
	...
	WHERE  CC.DCCCNID51=@EmpNode AND ID.COSTCENTERID=40072 AND ID.StatusID=369 AND ISDATE(TD.DCALPHA1)=1 AND ISDATE(DCALPHA3)=1 AND ISNULL(TD.DCALPHA2,'')<>'' AND ISNULL(TD.DCALPHA3,'')<>'' AND ISNULL(TD.DCALPHA1,'')<>'' AND CONVERT(DATETIME,TD.DCALPHA3)<CONVERT(DATETIME,@FromDate) ORDER BY  CONVERT(DATETIME,ISNULL(dcAlpha1,'')) desc))
	BEGIN
	...
	ELSE
	BEGIN
		SELECT TOP 1 @OPVACATIONDAYS= CASE WHEN isnull((convert(Float,dcAlpha11)),0) >0 THEN -1*isnull((convert(Float,dcAlpha11)),0) 
		...
		WHERE  CC.DCCCNID51=@EmpNode AND ID.COSTCENTERID=40072 AND ID.StatusID=369 
...
ELSE IF ((SELECT count(*)  FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DOCTEXTDATA TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID JOIN COM_DOCCCDATA CC  WITH(NOLOCK) ON ID.INVDOCDETAILSID=CC.INVDOCDETAILSID
		  WHERE  CC.DCCCNID51=@EmpNode AND ID.COSTCENTERID=40072 AND ID.StatusID=369 AND isnull(TD.DCALPHA16,'')='Yes' AND CONVERT(DATETIME,TD.DCALPHA3)<CONVERT(DATETIME,@FromDate))>0)
BEGIN
	IF(CONVERT(DATETIME,@OPLeavesAsOn)>=(SELECT TOP 1 CONVERT(DATETIME,TD.DCALPHA1) FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DOCTEXTDATA TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID JOIN COM_DOCCCDATA CC  WITH(NOLOCK) ON ID.INVDOCDETAILSID=CC.INVDOCDETAILSID
	WHERE  CC.DCCCNID51=@EmpNode AND ID.COSTCENTERID=40072 AND ID.StatusID=369 AND ISDATE(TD.DCALPHA1)=1 AND ISDATE(DCALPHA3)=1 AND isnull(TD.DCALPHA16,'')='Yes'  AND CONVERT(DATETIME,TD.DCALPHA3)<CONVERT(DATETIME,@FromDate) ORDER BY  CONVERT(DATETIME,ISNULL(dcalpha3,'')) desc))
	BEGIN
		SELECT @OPVACATIONDAYS=ISNULL(OPVACATIONDAYS,0),@OPVACATIONSALARY=ISNULL(OPVACATIONSALARY,0) FROM COM_CC50051 WITH(NOLOCK) WHERE NODEID=@EmpNode
	END
	ELSE
...
END	 
	...
	*/
	...
	SELECT @OPVACATIONDAYS AS OPVACATIONDAYS,@OPVACATIONSALARY AS OPVACATIONSALARY
...
IF ((SELECT count(*)  FROM INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DOCTEXTDATA TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID JOIN COM_DOCCCDATA CC  WITH(NOLOCK) 
ON ID.INVDOCDETAILSID=CC.INVDOCDETAILSID WHERE  CC.DCCCNID51=@EmpNode
 AND ID.COSTCENTERID=40072 AND ID.StatusID IN (371,441)
