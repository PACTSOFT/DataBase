...
SET NOCOUNT ON;  
		...
		@FCWhere NVARCHAR(20),@ExcTerminatedPDC NVARCHAR(20) ,@ExcRefPDC NVARCHAR(20),@isCrossDimRct BIT,@PDCFilterDimension INT
		...
		--WHERE CLAUSE
		set @WHERE='
		where  (D.DocumentType=19 or (D.DocumentType=16 and D.OpPdcStatus in(370,369,429))) and (D.LinkedAccDocDetailsID is null or D.LinkedAccDocDetailsID=0) and D.StatusID not in (371,372,441,447,448) and (D.DocOrder is null or D.DocOrder!=5) and D.'+@FilterOn+' Between '+convert(nvarchar,@From)+' and '+convert(nvarchar,@To)+@FCWhere
		if(@BankAccountID>0)
