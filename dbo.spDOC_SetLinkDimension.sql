...
SET NOCOUNT ON    
		Declare @DocumentLinkDefID bigint,@Srccol nvarchar(50),@Descol nvarchar(50),@i int,@ctn int,@isinv bit,@SrcTabName nvarchar(200),@ColID BIGINT
		...
		declare @tab table(id bigint identity(1,1) PRIMARY KEY,Srccol nvarchar(50),dtype  nvarchar(50),Descol nvarchar(50),tabName nvarchar(200),ColID BIGINT,[Decimal] int)   
		insert into @tab
		...
		BEGIN
			...
			BEGIN
				...
				if(@Costcenterid not between 40000 and 50000 and @Srccol like 'CCNID%')	
				...
				ELSE if(@Costcenterid=95 and @SrcTabName='REN_ContractParticulars' and @Srccol='RentAmount')
			...
			ELSE IF(@dtype='date' and @Descol like 'ccalpha%')
			BEGIN
				IF(@DimCCID=2)
				BEGIN
					IF isdate(@Val)=1
