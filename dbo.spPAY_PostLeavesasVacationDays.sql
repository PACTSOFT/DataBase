...
BEGIN
SET NOCOUNT ON;
DECLARE @PayrollProduct INT,@CrACC INT,@DbAcc INT,@CCID INT
DECLARE @TABVACATION TABLE(ID BIGINT IDENTITY(1,1),EMPNODE INT,EMPNAME VARCHAR(500),FROMDATE DATETIME,TODATE DATETIME)
DECLARE @TABLEAVESADJUSTMENT TABLE(ID BIGINT IDENTITY(1,1),INVDOCDETAILSID INT,EMPNODE INT,EMPNAME VARCHAR(500),FROMDATE DATETIME,TODATE DATETIME,NOOFDAYS DECIMAL(9,2),REMARKS VARCHAR(500))
...
BEGIN
	...
	WHERE   ID.COSTCENTERID=40072 AND C51.DCCCNID51=@EMPNODE AND ID.STATUSID=369 ORDER BY C51.DCCCNID51
	
	INSERT INTO @TABLEAVESADJUSTMENT
	SELECT  ID.INVDOCDETAILSID,TD.dcAlpha5,EMP.NAME,CONVERT(DATETIME,TD.dcAlpha2),CONVERT(DATETIME,TD.dcAlpha3),ISNULL(DN.DCNUM1,0),''
	FROM    INV_DOCDETAILS ID WITH(NOLOCK)
			INNER JOIN COM_DocCCData C51 WITH(NOLOCK) ON C51.INVDOCDETAILSID=ID.INVDOCDETAILSID	INNER JOIN COM_DocNumData DN WITH(NOLOCK) ON DN.INVDOCDETAILSID=ID.INVDOCDETAILSID
	...
	WHERE   ID.COSTCENTERID=40073 AND ID.STATUSID=369 AND TD.dcAlpha5=@EMPNODE AND ID.DOCID=@DOCID AND ISNULL(dcAlpha4,'')='Yes' ORDER BY TD.dcAlpha5
	...
	DECLARE @INVDOCDETAILSID INT,@INVDOCDETAILSIDNEW INT,@REJOINDATE VARCHAR(30),@NEWDOCID BIGINT
	...
	BEGIN
		...
		BEGIN
			...
			--CALCULATING VACATION DAYS-- DocID(-100) TO READ ONLY VACATION DAYS INFORMATION
				...
				SELECT @dcAlpha1=@REJOINDATE,@dcAlpha4=@VACATIONDAYS,@dcAlpha5=ISNULL(@OPVACATIONSALARY,0),@dcAlpha6=ISNULL(@OPVACATIONSALARY,0),@dcAlpha7=@CREDITEDDAYS,@dcAlpha8=@VACATIONDAYS,@dcAlpha9=@VACATIONDAYS,@dcAlpha10=@VACATIONDAYS,
					   @dcAlpha11=@EXCESSDAYS,@dcAlpha12=@REMAININGDAYS,
