...
SET NOCOUNT ON   
		--Declaration Section
			...
			BEGIN
			set @Name=''
			select @Name=ServiceName  from CRM_ServiceTypes where ServiceName =@ServiceName 
			if (@Name =@ServiceName)
			BEGIN
				RAISERROR('-112',16,1)
			END
			...
			BEGIN
				...
				BEGIN
					...
					SET @AttachmentID=(Select COUNT(fileid) from COM_Files where FeatureID=82 and FeaturePK=@ServiceTypeID)
			...
			IF @AttachmentID=0 
					BEGIN
						...
						SET @AttachmentID=(Select TOP 1 fileid from COM_Files where FeatureID=82 and FeaturePK=@ServiceTypeID)
					END
