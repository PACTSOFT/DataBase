...
SET NOCOUNT ON;  
	...
	DECLARE @EMPID INT,@LEAVETYPE INT,@FromDate DATETIME,@TDate DATETIME,@DocLoc INT,@actDocDate INT,@AssignLeaves float,@LeaveYear varchar(5),@LoctID INT,@DailyattendanceDate1 datetime,@ActXml nvarchar(max)
			...
			BEGIN  
				...
				BEGIN
					...
					BEGIN
						...
						SET @SQL='SELECT @ind=COUNT(*) FROM Inv_DocDetails ID  WITH(NOLOCK) 
		   				JOIN Com_DocTextData TD WITH(NOLOCK) ON TD.InvDocDetailsID=ID.InvDocDetailsID
		   				WHERE TD.tCOSTCENTERID=40074 AND TD.dcAlpha1='''+@EMPYEAR+''' AND ID.DOCID<>'+CONVERT(NVARCHAR,@DOCID)
						EXEC sp_executesql @SQL,N'@ind INT OUTPUT',@ind OUTPUT
