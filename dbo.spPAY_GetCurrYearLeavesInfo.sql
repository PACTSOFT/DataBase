		...
		END
			...
			BEGIN			
				...
				BEGIN
					...
					WHERE  ISNUMERIC(TD.DCALPHA3)=1 AND CONVERT(DATETIME,ID.DOCDATE) <=CONVERT(DATETIME,@LVStartDate)
				...
				--START :CURRENT DATERANGE LEAVES TAKEN
				    ...
				    --END: LOADING DATES FROM @MONTHTAB TABLE
						--START: LOADING DATA BASED ON FROMDATE AND TODATE GIVEN 
						  DECLARE @DATESAPPLIEDCOUNT TABLE (FDATE DATETIME,TDATE DATETIME,STODATE DATETIME,EODATE DATETIME,NOOFDAYS DECIMAL(9,2),PostedFrom NVARCHAR(100))
					   ...
					   --START: UPDATING @DATESCOUNT TABLE 'COUNT' COLUMN TO 1 FROM LIST OF @DATESAPPLIEDCOUNT TABLE
						  ...
						  BEGIN
								...
								SELECT @RC=COUNT(*) FROM @DATESAPPLIEDCOUNT WHERE CONVERT(DATETIME,@DTT) between CONVERT(DATETIME,FDATE) and CONVERT(DATETIME,TDATE) AND PostedFrom<>'LatesProcessing'
				...
				--START WEEKLYOFF COUNT
					...
					WHERE        DC.INVDOCDETAILSID=ID.INVDOCDETAILSID AND TD.INVDOCDETAILSID=ID.INVDOCDETAILSID AND ID.COSTCENTERID=40053 AND DC.dcCCNID51=@EmployeeID	
								 AND CONVERT(DATETIME,ID.DUEDATE)<=CONVERT(DATETIME,@ALStartMonthYear)
					ORDER BY     CONVERT(DATETIME,ID.DUEDATE) DESC

					SELECT TOP 1  @TD=CONVERT(DATETIME,ID.DUEDATE)
								 FROM COM_DocCCData DC WITH(NOLOCK),INV_DOCDETAILS ID WITH(NOLOCK),COM_DocTextData TD WITH(NOLOCK)
					WHERE        DC.INVDOCDETAILSID=ID.INVDOCDETAILSID AND TD.INVDOCDETAILSID=ID.INVDOCDETAILSID AND ID.COSTCENTERID=40053 AND DC.dcCCNID51=@EmployeeID	
								 AND CONVERT(DATETIME,ID.DUEDATE)<=CONVERT(DATETIME,@ALEndMonthYear)
					ORDER BY     CONVERT(DATETIME,ID.DUEDATE) DESC
...
-----------------------------
				...
				SELECT @CurrYearLeavesTakenOP=COUNT(*) FROM @DATESCOUNT WHERE FLAG=1 AND count>=1 and isnull(IncExc,'')='' and convert(datetime,date1) between convert(datetime,@FromDate) and convert(datetime,@ToDate)
				SET @CurrYearLeavesTakenOP=isnull(@CurrYearLeavesTakenOP,0)-isnull(@DAYSSUM,0)
				
				declare @LPCnt FLOAT 
				SET @LPCnt=0
				SELECT @LPCnt=SUM(NoOfDays) FROM @DATESAPPLIEDCOUNT 
				WHERE PostedFrom='LatesProcessing' 
				AND FDATE IN( 
				SELECT DATE1 FROM @DATESCOUNT --WHERE FLAG=1 AND count>=1 and isnull(IncExc,'')=''
				)
