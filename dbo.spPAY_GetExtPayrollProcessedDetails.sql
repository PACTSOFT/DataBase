...
BEGIN
	...
	BEGIN	
		...
		ELSE IF(@DOCUMENTTYPE=97)
			SELECT @FROMDATE= MAX(CONVERT(DATETIME,DCALPHA16)) FROM INV_DOCDETAILS ID WITH(NOLOCK) INNER JOIN COM_DOCTEXTDATA TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID INNER JOIN COM_DOCCCDATA CC WITH(NOLOCK) ON ID.INVDOCDETAILSID=CC.INVDOCDETAILSID	 WHERE ID.DOCID=@DOCID AND CC.DCCCNID51=@EMPNODE
		ELSE
