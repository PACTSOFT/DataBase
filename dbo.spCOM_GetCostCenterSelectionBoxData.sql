...
SET NOCOUNT ON    
  ...
  begin
	...
	BEGIN
		...
		if exists(select IsInventory from ADM_DocumentTypes where CostCenterID=@CostCenterID and IsInventory=1)
		  set  @SQL=@SQL+' join INV_DocDetails i on i.InvDocDetailsID=d.InvDocDetailsID
			 where i.CostCenterID='+CONVERT(nvarchar,@CostCenterID)
		else
			set  @SQL=@SQL+' join ACC_DocDetails a on a.AccDocDetailsID=d.AccDocDetailsID
	...
	BEGIN
		...
		if exists(select IsInventory from ADM_DocumentTypes where CostCenterID=@CostCenterID and IsInventory=1)
		  set  @SQL=@SQL+'  INV_DocDetails where CostCenterID='+CONVERT(nvarchar,@CostCenterID)
		else
			set  @SQL=@SQL+'  ACC_DocDetails where CostCenterID='+CONVERT(nvarchar,@CostCenterID)
