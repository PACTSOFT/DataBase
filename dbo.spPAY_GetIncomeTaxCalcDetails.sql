...
Select td.dcalpha1 as ITYear,td.dcalpha6 as MetroFormula,td.dcalpha7 as NonmetroFormula,td.dcalpha2 as TotalIncomeRoundoff,td.dcalpha3 as Surcharge,td.dcalpha4 as EducationCess,td.dcalpha5 as SecondaryandHighEduc,
	   td.dcalpha8 as Gender,dn.dcnum1 as Fromslab,dn.dcnum2 as Toslab,dcnum3 as Percentage from INV_DOCDETAILS ID WITH(NOLOCK) JOIN COM_DOCTEXTDATA TD WITH(NOLOCK) ON ID.INVDOCDETAILSID=TD.INVDOCDETAILSID JOIN COM_DOCNUMDATA DN WITH(NOLOCK) ON ID.INVDOCDETAILSID=DN.INVDOCDETAILSID
...
SELECT	 PARENTID,NODEID,REPLACE(NAME,'IT_','') NAME,ISGROUP, CCALPHA2 AS FILEDTYPE FROM  COM_CC50052 WITH(NOLOCK) WHERE  isnull(CCALPHA3,0)<=0 AND ISGROUP=0
		 ...
		 AND CCALPHA2='Income'	GROUP BY NODEID,NAME,PARENTID,ISGROUP,CCALPHA2
...
--14.
DECLARE @TABHOUSEPROPERTY TABLE(ID INT IDENTITY(1,1),ORDERNO INT,PARENTID INT,NODEID INT,NAME VARCHAR(500),AMOUNT FLOAT)
...
INSERT INTO @TABHOUSEPROPERTY
	SELECT	 CCALPHA3 ORDERNO,PARENTID,NODEID,NAME,ETD.Amount FROM COM_CC50052 c52 WITH(NOLOCK),PAY_EmpTaxDeclaration ETD with(nolock) WHERE ETD.componentid=c52.nodeid and ETD.empnode=@EmpSeqNo and ETD.year=@Year and isnull(CCALPHA3,0)>0
	  		 And ccAlpha4='House Property/Other Source' Order By orderno
END	  		 
ELSE
BEGIN
INSERT INTO @TABHOUSEPROPERTY
	 SELECT	 CCALPHA3 ORDERNO,PARENTID,NODEID,NAME,0 FROM COM_CC50052 c52 WITH(NOLOCK) WHERE isnull(CCALPHA3,0)>0 And ccAlpha4='House Property/Other Source' Order By orderno
END
...
--16.80C and 80CCC
DECLARE @TAB80SECTION TABLE(ID INT IDENTITY(1,1),ORDERNO INT,PARENTID INT,NODEID INT,NAME VARCHAR(500),AMOUNT FLOAT)
...
INSERT INTO @TAB80SECTION
	   SELECT convert(int,CCALPHA3) ORDERNO,PARENTID,NODEID,NAME,etd.amount FROM COM_CC50052 c52 WITH(NOLOCK),PAY_EmpTaxDeclaration ETD with(nolock)
	   WHERE ETD.componentid=c52.nodeid and ETD.empnode=@EmpSeqNo and ETD.year=@Year And ccAlpha4='80C' Order By ParentID,OrderNo
END
ELSE
BEGIN
INSERT INTO @TAB80SECTION
	SELECT convert(int,CCALPHA3) ORDERNO,PARENTID,NODEID,NAME,0 FROM COM_CC50052 c52 WITH(NOLOCK)   WHERE  ccAlpha4='80C' Order By ParentID,OrderNo
END

SELECT * FROM @TAB80SECTION

--17.OTHER THAN 80C SECTIONS
DECLARE @TABCHAPTERADETAILS table(ID INT IDENTITY(1,1),ORDERNO INT,PARENTID INT,NODEID INT,NAME VARCHAR(500),AMOUNT FLOAT)				
INSERT INTO @TABCHAPTERADETAILS				
	   SELECT convert(int,CCALPHA3) ORDERNO,PARENTID,NODEID,NAME,0 FROM COM_CC50052 c52 WITH(NOLOCK) WHERE ccAlpha4='Otherthan80C' Order By NodeID ,OrderNo
...
DECLARE @ExemUS10 table(ID INT IDENTITY(1,1),ORDERNO INT,PARENTID INT,NODEID INT,NAME VARCHAR(500),AMOUNT FLOAT)				
INSERT INTO @ExemUS10				
	   SELECT convert(int,CCALPHA3) ORDERNO,PARENTID,NODEID,NAME,0 FROM COM_CC50052 c52 WITH(NOLOCK) WHERE ccAlpha4='Exemption U/S 10' Order By NodeID ,OrderNo
