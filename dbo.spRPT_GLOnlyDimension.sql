...
/***16-Opening Balance,14-Postdated Payment,19-Postdated Receipts***/
	...
	DECLARE	@From NVARCHAR(20),@To NVARCHAR(20),@AmtColumn NVARCHAR(10),@CurrWHERE1 NVARCHAR(30),@CurrWHERE2 NVARCHAR(30)
...
D.'+@AmtColumn+' DebitAmount,NULL  CreditAmount,0.0 Balance,D.ChequeNumber,CONVERT(DATETIME,D.ChequeDate) ChequeDate,CONVERT(DATETIME,D.ChequeMaturityDate) ChequeMaturityDate,0 VType,D.StatusID'+@SELECTQUERY+'
FROM ACC_DocDetails D with(nolock) INNER JOIN ACC_Accounts A with(nolock) ON A.AccountID=D.DebitAccount'+@AccountTypes+'
...
NULL DebitAmount,D.'+@AmtColumn+' CreditAmount,0.0 Balance,D.ChequeNumber,CONVERT(DATETIME,D.ChequeDate) ChequeDate,CONVERT(DATETIME,D.ChequeMaturityDate) ChequeMaturityDate,0 VType,D.StatusID'+@SELECTQUERY+'
FROM ACC_DocDetails D with(nolock) INNER JOIN ACC_Accounts A with(nolock) ON A.AccountID=D.CreditAccount'+@AccountTypes+'
...
AD.'+@AmtColumn+' DebitAmount,NULL  CreditAmount,0.0 Balance,AD.ChequeNumber,CONVERT(DATETIME,AD.ChequeDate) ChequeDate,CONVERT(DATETIME,AD.ChequeMaturityDate) ChequeMaturityDate,0 VType,D.StatusID'+@INV_SELECT+'
...
NULL DebitAmount,AD.'+@AmtColumn+' CreditAmount,0.0 Balance,AD.ChequeNumber,CONVERT(DATETIME,AD.ChequeDate) ChequeDate,CONVERT(DATETIME,AD.ChequeMaturityDate) ChequeMaturityDate,0 VType,D.StatusID'+@INV_SELECT+'
...
BEGIN
	...
	D.'+@AmtColumn+' DebitAmount,NULL CreditAmount,0.0 Balance,D.ChequeNumber,CONVERT(DATETIME,D.ChequeDate) ChequeDate,CONVERT(DATETIME,D.ChequeMaturityDate) ChequeMaturityDate,D.DocumentType VType,D.StatusID StatusID'+@SELECTQUERY+'
	FROM ACC_DocDetails D with(nolock) INNER JOIN ACC_Accounts A with(nolock) ON A.AccountID=D.DebitAccount'+@AccountTypes+'
	...
	NULL DebitAmount,D.'+@AmtColumn+' CreditAmount,0.0 Balance,D.ChequeNumber,CONVERT(DATETIME,D.ChequeDate) ChequeDate,CONVERT(DATETIME,D.ChequeMaturityDate) ChequeMaturityDate,D.DocumentType VType,D.StatusID StatusID'+@SELECTQUERY+'
	FROM ACC_DocDetails D with(nolock) INNER JOIN ACC_Accounts A with(nolock) ON A.AccountID=D.CreditAccount'+@AccountTypes+'
	INNER JOIN COM_DocCCData DCC with(nolock) ON DCC.AccDocDetailsID=D.AccDocDetailsID '+@FROMQUERY+'
	WHERE '+@Temp+' AND (D.DocumentType=14 OR D.DocumentType=19) AND D.'+@PDCFilterOn+'<='+@To+' '+@LocationWHERE+@CurrWHERE1+@LineWisePDC
