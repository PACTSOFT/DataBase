...
Declare @GetAvlblLeaves TABLE(Id Int Identity(1,1),AssignedLeaves INT,AvailableLeaves DECIMAL(9,2),FDate DateTime,TDate DateTime,MaxEncashLeaves Float,MaxLeaves Float,ActualAvailableLeaves float,ThresholdLimit float)
Declare @TabLeaveTypes Table(Sno Int Identity(1,1),LeaveTypeNode Int,AvlblLeaves Float)
Declare @MonthDays TABLE(ID Bigint Identity(1,1),FDATE DateTime,TDATE DateTime,TOTALDAYS FLOAT,ACTUALDAYS FLOAT,DAYS float,LEAVESTAKEN FLOAT,LOPDAYS FLOAT)
...
--LOADING OTHER THAN VACATION LEAVETYPE LEAVES
Insert Into @TabLeaveTypes 
	Select Distinct cc.dccCNID52,0 From Inv_DocDetails ID WITH(NOLOCK),Com_DocCCData CC  WITH(NOLOCK) Where ID.InvDocDetailsID=CC.InvDocDetailsID And ID.CostCenterID=40060 And CC.dccCNID51=@Empnode
Set @Rc=1
Select @Trc=Count(*) from @TabLeaveTypes
While(@Rc<=@Trc)
Begin
	Select @LeaveType=LeaveTypeNode From @TabLeaveTypes Where Sno=@Rc
	
	Insert Into @GetAvlblLeaves
	Exec spPAY_ExtGetAssignedLeaves @Empnode,@LeaveType,@Date
	
	Select @AvblblLeaves=AvailableLeaves from @GetAvlblLeaves where Id=@Rc
	Update @TabLeaveTypes Set AvlblLeaves=@AvblblLeaves Where Sno=@Rc
Set @Rc=@Rc+1
End
...
WHERE  a.CostCenterID=40072 and a.StatusID=369 AND b.dcCCNID51=@Empnode 
...
---------------------------------------------------
	...
	IF EXISTS ( 
				...
				WHERE I.CostCenterID=40095 AND dcAlpha1='2' and i.StatusID=369 AND cc.dcCCNID51=@Empnode 
				)
				BEGIN
...
UNION
SELECT @AssingedLeaveType as LeaveType,Round(isnull(@TotalDays,0),2) CreditedDays,ISNULL(@OPVACATIONDAYS,0) as OPBalOrRemainingDays,@LeavesTakenEncashSUM, Round(ISNULL(@OPVACATIONDAYS,0)+isnull(@TotalDays,0)-isnull(@LeavesTakenEncashSUM,0),2) AS AvlblDays,Round(ISNULL(@OPVACATIONDAYS,0)+isnull(@CrDaysUptoLastMonth,0),2)
