...
WHERE C.StatusID not in (451,440,466) AND convert(datetime,C.EndDate) <= @todate and (C.statusid=427 or C.statusid=426) AND C.RefContractID=0 '+@WHERE1+'
union all
SELECT distinct C.contractid,C.Sno, 
P.Name Property,U.Name Unit,P.NodeID [Property_ID],U.UnitID [Unit_ID],T.TenantID [Tenant_ID],
T.FirstName Tenant,T.Phone1 Phone1,
...
left join REN_ContractParticulars CP with(nolock) on C.CONTRACTID=CP.CONTRACTID  and CP.CCNodeID=3 '+@FromTag+'
join (select UnitID,max(EndDate) EndDate from
		(select C.UnitID,max(ISNULL(ISNULL(C.TerminationDate,c.RefundDate),C.EndDate)) EndDate from ren_contract C with(nolock) left join ren_contract CU with(nolock) on CU.RefContractID=C.ContractID where CU.ContractID is null and c.statusid NOT IN (451,440) group by C.UnitID
		union all
		select CU.UnitID,max(ISNULL(ISNULL(C.TerminationDate,c.RefundDate),C.EndDate)) EndDate from ren_contract C with(nolock) inner join ren_contract CU with(nolock) on CU.RefContractID=C.ContractID where c.statusid NOT IN (451,440) group by CU.UnitID) as T
	group by UnitID) T6 ON (CU.UnitID is not null and T6.UnitID=CU.UnitID and T6.EndDate=C.EndDate) or (T6.UnitID=C.UnitID and T6.EndDate=C.EndDate)
WHERE C.StatusID not in (451,440,466) AND convert(datetime,C.EndDate) <= @todate and  (C.statusid=427 or C.statusid=426) AND C.RefContractID=0 '+@WHERE1+' )
AS T
GROUP BY contractid
...
SELECT P.Name PName,U.Name UName,T.LeaseSignatory LeaseSignatory,T.Email TEmail,T.Phone1 TPhone1,T.Fax TFax,
...
DECLARE @TAB TABLE(ID INT IDENTITY(1,1) PRIMARY KEY,UnitID INT,CurrentRent FLOAT,Status NVARCHAR(10),NoofDays INT,VacantSince DATETIME,ContractID INT,DiscountAmount FLOAT)
...
BEGIN
	...
	,@VacantSince=(CASE WHEN StatusID = 450 THEN CONVERT(DATETIME,ISNULL(RefundDate,VacancyDate)) WHEN StatusID = 480 THEN CONVERT(DATETIME,VacancyDate) 
			ELSE CONVERT(DATETIME,TerminationDate) END )
	,@NoofDays=(CASE WHEN StatusID = 450 THEN DATEDIFF(DAY, CONVERT(DATETIME,ISNULL(RefundDate,VacancyDate)),CONVERT(DATETIME,@FROM)) 
			WHEN StatusID = 480 THEN DATEDIFF(DAY, CONVERT(DATETIME,VacancyDate),CONVERT(DATETIME,@FROM)) 
	...
	UPDATE @TAB SET CurrentRent=@TotalAmount,NoofDays=@NoofDays,VacantSince=@VacantSince,ContractID=@ContractID,DiscountAmount=@DiscountAmount WHERE ID=@I
	
	IF @StatusID=427 OR @StatusID=426 OR @StatusID=466
	BEGIN
		UPDATE @TAB SET Status=''Occupied'' WHERE ID=@I
	END
...
SELECT TEMP.*,P.NodeID Building_ID,P.NAME Building,TEMP.UnitID UnitNo_ID,U.NAME UnitNo,C18.NAME UnitType,U.UnitStatus,U.RentableArea UnitArea,U.RentPerSQFT UnitRate,U.AnnualRent EstimatedRent
,CONVERT(DATETIME,T1.StartDate) NewStartDate '+@SelectTag+'
FROM @TAB TEMP
JOIN REN_Units U WITH(NOLOCK) ON TEMP.UnitID=U.UnitID
JOIN REN_Property P WITH(NOLOCK) ON P.NodeID=U.PropertyID
LEFT JOIN (SELECT UnitID,MIN(StartDate) StartDate FROM REN_Contract WITH(NOLOCK) WHERE StatusID<>451 AND StartDate>=@FROM GROUP BY UnitID) AS T1 ON T1.UnitID=U.UnitID
...
'+REPLACE(REPLACE(REPLACE(REPLACE(@FromTag,'C.UnitID','U.UnitID'),'C.PropertyID','U.PropertyID')
,'JOIN REN_ContractExtended','LEFT JOIN REN_ContractExtended')
,'JOIN REN_TenantExtended','LEFT JOIN REN_TenantExtended')
IF @ReportType=212
...
SELECT Property,MAX([Property_ID]) [Property_ID],UnitType,COUNT(*) NoOfUnits'+@MaxSelectTag+'
