...
INSERT INTO #TABDATDIMENSIONS 
	SELECT USERCOLUMNNAME,SYSCOLUMNNAME,40067 FROM ADM_COSTCENTERDEF WHERE COSTCENTERID=40067 AND SYSCOLUMNNAME LIKE 'dcCCNID%' AND USERCOLUMNNAME NOT LIKE 'dcCCNID%' AND SYSCOLUMNNAME <>'dcCCNID51' AND SectionID=3	        
	
INSERT INTO #TABATTDIMENSIONS 
	SELECT USERCOLUMNNAME,SYSCOLUMNNAME,40079 FROM ADM_COSTCENTERDEF WHERE COSTCENTERID=40079 AND SYSCOLUMNNAME LIKE 'dcCCNID%' AND USERCOLUMNNAME NOT LIKE 'dcCCNID%' AND SYSCOLUMNNAME <>'dcCCNID51' AND SectionID=3	        
...
--LOADING DATA FROM DAILY ATTENDANCE WITH DIMENSIONS
	...
	SET @strQry=@strQry+'INSERT INTO @TABATTENDANCEDIM1
							...
							WHERE    ID.INVDOCDETAILSID=TD.INVDOCDETAILSID AND ID.INVDOCDETAILSID=DN.INVDOCDETAILSID AND ID.INVDOCDETAILSID=CC.INVDOCDETAILSID
								     AND CC.DCCCNID51=C51.NODEID AND ID.DocumentType=67 AND ID.StatusID=369 AND CC.DCCCNID51 in ('+ @EmpIDs+') 
							...
							FROM     INV_DOCDETAILS ID WITH(NOLOCK),COM_DOCTEXTDATA TD WITH(NOLOCK),COM_DOCNUMDATA DN WITH(NOLOCK),COM_DOCCCDATA CC WITH(NOLOCK),COM_CC50051 C51 WITH(NOLOCK)
							WHERE    ID.INVDOCDETAILSID=TD.INVDOCDETAILSID AND ID.INVDOCDETAILSID=DN.INVDOCDETAILSID AND ID.INVDOCDETAILSID=CC.INVDOCDETAILSID
								     AND CC.DCCCNID51=C51.NODEID AND ID.CostCenterID=40062 AND ID.StatusID=369 AND CC.DCCCNID51 in ('+ @EmpIDs+') 
	...
	BEGIN
		...
		WHERE a.CostCenterID=40062 and a.StatusID=369 AND b.dcCCNID51 in ('+ @EmpIDs+') 
