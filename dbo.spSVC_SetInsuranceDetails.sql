...
GO
CREATE PROCEDURE [dbo].[spSVC_SetInsuranceDetails]
	@NodeID [bigint] = 0,
	@Category [bigint],
	@Insurance [bigint],
	@Location [bigint],
...
SET NOCOUNT ON;
	...
	--	DECLARE @lft bigint,@rgt bigint,@Depth bigint,@ParentID bigint,@IsGroup bigint
        ...
        --set @NodeID=(select NodeID from SVC_InsuranceDetails where category=@Category and WEF=@WEF)
		...
		IF @NodeID=0
		BEGIN
			if ((select count(*) from dbo.SVC_InsuranceDetails  where category=@Category 
			and Location=@Location and Insurance=@Insurance and WEF=@wefc)>0)
			BEGIN
				RAISERROR('-343',16,1)
			END
			insert into SVC_InsuranceDetails(category,Insurance, Location,WEF,Type,ProductPercentage,ProductAmount,
				CompanyGUID,GUID,CreatedBy,CreatedDate,LabType,LabPercentage, LabAmt)
			values(@Category,@Insurance, @Location,@wefc,@Type,@Per,@Amt,
		...
		ELSE
		BEGIN
			UPDATE SVC_InsuranceDetails
			SET Category=@Category, Insurance=@Insurance,
			Location=@Location,WEF=@wefc,Type=@Type,
